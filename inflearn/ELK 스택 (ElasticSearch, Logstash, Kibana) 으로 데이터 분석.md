...
- management에서 인덱스를 기반으로 데이터를 생성하는것같음
- discover탭에서 차트, 데이터 조회
- visualize에서 차트 생성 vertial bar chart, pie chart ..
- 매핑을 해줌으로써 키바나에게 데이터 타입을 알려주는 것이 중요함
- visualize에서 생성된 데이터는 dashboard에서 사용할 수 있음
...
- 로그스테시는 인풋을 담당함
- 로그스테시에서 받은 인풋은 엘라로 들어가고 키바나가 시각화함
- 로그스테시는 mongodb, mysql 등등 어댑터가 전부 있어서 연결할 수 있음
- 로그스테시는 데이터를 알맞게 변환시키고 엘라로 보냄



# 섹션 5. 실전 ELK 활용한 실데이터 분석

# 섹션 6. ELK 클라우드 로그분석 시스템

- elk스택으로 중앙 집중 로그 분석을 구현할 수 있음
- 파일비트를 사용해서 로그파일에 변화가 있을때 로그스테시로 전달함
- 데이터사이즈 문제는 큐레이터를 사용하면 됨
- 
