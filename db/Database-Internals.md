...
- 페이지 캐시는 순서를 고려하지 않기 떄문에 디스크와 메모리에서 페이지가 정렬되는 방식 사이에는 연관성이 없다.
- 페이지 캐시의 주요 기능
  - 페이지 내용을 메모리에 캐시한다.
...
- 가까운 시간 내에 요청될 확률이 높은 페이지는 고정시킬 수 있다. 고정된 페이지는 메모리에 더 오랜 시간 유지되기 떄문에 디스크 접근 횟수가 줄어들고 성능에 도움이 된다.
- 트리의 하위 레벨에는 상위 레벨보다 노드 수가 월등히 많기 떄문에 상위 레벨 노드는 트리의 전체에서 극히 일부분에 해당한다. 상위 레벨 노드를 메모리에 고정시키고 나머지 노드는 요청 시 페이징해도된다.
