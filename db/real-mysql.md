...
- MYSQL 에서 IN 연산자는 사용법에 따라 상당히 비효율적으로 처리될 떄도 있음 다른 DBMS는 최적화해서 수행함
...
- 프리페어 스테이트먼트를 제외하면 SQL은 텍스트 기반으로 작동하기 떄문에 SQL에 포함된 모든 입력 값은 문자열처럼 취급됨
...
- AUTO_INCREMENT 사용시 규칙
  ...
  - AUTO_INCREMENT 속성을 가진 칼럼 하나로 PK를 생성할 떄는 아무런 제약이 없음
  - 여러개의 칼럼으로 PK를 생성할 때
    - AUTO_INCREMENT 속성의 칼럼이 제일 앞일 떄
      - 아무런 제약 없음
...
- 이 잠금은 현재 값을 가져올 때만 잠금이 걸렸다가 즉시해제됨 테이블 락이지만 성능상 문제될 떄는 거의 없음
...
- 이떄 INSERT 문장에 IGNORE 옵션만 추가하면 중복 에러가 발생해도 그냥 무시하고 계속 진행함
- INSERT IGNORE 문장에서 에러가 발생할 떄와 발생하지 않을 때
