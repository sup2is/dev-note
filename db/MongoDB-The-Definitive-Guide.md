...
- writeConcern 과 결합하면 애플리케이션에 대한 일관성과 가용성 보장을 제어할 수 있따.
  - local
  - available
  - majority
  - linearizable
...
- 권장되는 일반적인 구성
  ...
  - 각 데이터 센터 내 서버의 개수가 동일하고 또 다른 위치에 과반수가 있는 서버의 구성
...
- 몽고DB는 버전 3.2에서 복제 프로토콜 버전 1을 도입했다. 
  ...
  - 가장큰 특징은 RAFT와 유사하다.
    - RAFT알고리즘: 비교적 독립적인 하위 문제로 분리되는 합의 알고리즘. 합의는 여러 서버나 프로세스가 값에 동의하는 과정. RAFT는 동일한 일련의 명령이 동일한 일련의 결과를 생성하고 배포 멤버 전체에서 동일한 일련의 상태에 도달하도록 합의를 보장한다.
...
- 특정 멤버가 우선적으로 프라이머리가 되게 하거나 클라이언트에 보이지 않개 해 읽기 요청이 라우티오디지 않도록 할 수 있다.
...
- 아비터를 추가할때는 반드시 멤버가 짝수인 상황에서 홀수가 되는 방향일 때 추가해야 한다.
...
- 조금 과부하된 상태로 다양한 상황에서 나타난다.
  ...
  - 또는 조각 모음 같은 긴 연산이 진행될 떄나 replSetMaintenance 명령에 대한 응답으로 RECOVERING 상태가 될 수 있다.
...
- 네트워크 상태가 양호하고 서버의 과반수가 살아 있으면 선출은 빠르게 진행되지만 네트워크에 문제가 있거나 너무 느리게 응답하는 과부하 서버 때문에 선출이 발생할 수도 있따. 이때 하트비트는 타임아웃 떄문에 시간을 더많이 소요할 수 있다. (최대 몇 분까지)
...
- w를 1로 설정하면 w옵션을 전혀 전달하지 않을 떄와 같으며 단지 프라이머리에서 쓰기가 성공했는지 확인한다.
...
- 드라이버에서 읽기 선호도를 설정해 다른 옵션을 구성할 수도 있따.
...
- 읽기 선호도
  ...
  - 드라이버에서 복제 셋 멤버까지의 평균 핑 시간을 기반으로 지연율이 가장 낮은 멤버에 요청을 라우팅할 수 있따.
...
- 수동 샤딩
  - 수동 샤딩은 어떤 데이터베이스 소프트웨어를 사용하든 대부분 수행할 수 있따.
  - 수동 샤딩을 사용하면 애플리케이션은 여러 데이터베이스 서버와 연결을 유지하고 각기 다른 데이터를 다른 서버에 저장하고 데이터를 가져오기 위해 적절한 서버에 쿼리하는 과정을 관리한다.
  - 수동 샤딩은 잘 동작하지만 클러스터에서 노드를 추가하거나 삭제할 때 혹은 데이터 분산이나 부하 패턴이 변화할 떄는 유지하기 어렵다.
- 자동 샤딩
...
- mongos
  ...
  - mongos는 어떤 데이터가 어떤 샤드에 있는지 알기 떄문에 요청을 적절한 샤드로 전달할 수 있따.
  - 요청에 대한 응답이 있으면 라우터는 응답을 수집하고 통합해 애플리케이션으로 되돌려 보낸다.

![14-1](/Users/a10300/Choi/Git/dev-note/db/14-1.jpeg)
...
- ShardingTest
  ...
  - ShardingTest는 컴퓨터에 /data/db 디렉토리가 있다고 가정한다.
...
- ShardingTest가 클러스터 설정을 마치면 연결 가능한 실행중인 10개이고 아래와 같다.
  - 노드 3개로 구성된 복제 셋 2개
...
- 몽고DB 4.2에서 autosplit 기능을 샤드 프라이머리 mongod로 이동함으로써 오름차순 샤드 키 패턴 문제를 해결하는 최상위 청크 최적화가 추가됐따. 배런서는 청크를 배치할 다른 샤드를 선택한다. 이는 새 청크가 모두 단 하나의 샤드에 생성되는 상황을 방지한다.
