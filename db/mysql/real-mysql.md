...
- ORDER BY가 없다고해서 랜덤이 아님

### 지금까지의 구현

- RAND()로 구현
- WHERE로 정렬 레코드를 많이 줄일 수 있으면 다행인데 아니라면 성능 + 자원 많이 소모함



### 인덱스를 이용한 임의 정렬

- 별도로 랜덤에 사용할 칼럼을 추가하고 인덱스를 걸면됨

```sql
SELECT * FROM tb_member
WHERE rand_val => FLOOR((RAND() * 100000000))
ORDER BY rand_val ASC LIMIT 3;
```

- 위 쿼리는 결과가 0일수도 있음 따라서 상위, 하위를 UNION ALL로 가져오면됨
...
- MYSQL에서 LIMIT을 많이 사용함 LIMIT가 사용된 페이징 쿼리가 인덱스를 이용할 수 있다면 크게 성능상의 문제 없이 사용 가능
- MYSQL LIMIT이라고하더라도 100페이지의 경우 1~1000번까지 읽고 10개씩 끊어서 읽음 뒤로 갈수록 더 심화됨



### 지금까지의 방법

- 커버링 인덱스로 된다면 그나마 다행인데 커버링 인덱스가 아니면 심지어 디스크에서 한번 읽어오게됨

### 불필요한 접근을 제거하기 위한 페이징 쿼리

- PK값을 WHERE절에 넣어주고 LIMIT을 걸면됨
- 이렇게되면 검색조건에의해 필터링된 요소에서 LIMIT을 걸기 때문에 디스크 일기 부하를 상당량 줄일 수 있음
- MYISAM의 경우 PK가 보조 인덱스에 자동으로 추가되지 않는점 참고
- 하지만 검색조건에서 LIKE나 BETWEEN을 추가해서 인덱스를 못태우면 검색, 정렬 모두 인덱스를 사용 불가하니 참고



## MySQL에서 시퀀스 구현

- 오라클의 시퀀스와 mysql의 auto_increment로 두가지가 있음
- 가끔 오라클의 시퀀스와 같이 특정 테이블에 의존적이지 않고 독립적으로 일련번호만 발급하는 기능이 mysql에 필요할 때가 있음
...
- 스토어드 함수로 캡슐화해서 구현해두는 편이 좋음
- 핵심은 mysql_sequences 테이블에서 레코드를 insert하고 만약 pk가 중복한다면 값을 1만큼 증가시켜서 저장함
...
- 시퀀스 테이블은 절대 트랜잭션을 지원하는 스토리지 엔진을 사용하면 안됨
- 복제시에도 주의해야함
...
- 매우 긴 문자열을 저장하는 칼럼에 대해 인덱스를 생성하거나 검새을 수행해야할 때
- 이럴땐 긴 문자열의 해시 값으로 프라이머리 키나 유니크 인덱스를 생성하는 방법을 사용하면 쉽게 해결 가능
...
- 키 파티션은 pk나 유니크키 칼럼을 이용해 파티션을 하는 방법임
- 5.15 이상부터 pk나 유니크 키의 일부만으로 키 파티션을 구현할 수 있게 됨



## sns의 타임라인 구현

- 사실 타임라인 기능은 rdb에 적합하지 않은 데이터형임
  - rdb는 쓰기가 느리지만 읽기는 빠름 but 타임라인은 쓰기비중이 더 높음
  - 타임라인기능은 인덱스 사용 불가
- nosql쓰면됨 ..ㅋ
...
- mysql 기본적으로 스케일아웃형식으로 확장하면서 성능을 높임
- 표준 설정 파일을 만들어두면 두고두고 쓸 
- 자세한건 스킵
...
- mysql에서 복제를 사용하려면 바이너리 로그 파일이 활성화되어야 함
- 바이너리 로그는 상당히 고비용 디스크 io를 필요로함
- 바이너리로그 갭락, 넥스트 키락을 사용함
- 복제를 안쓰면 바이너리로그를 비활성화하고 트랜잭션 격리 수준을 read-commited로 사용하는것이 가장 좋음 하지만 이렇게 설정해도 pk에 대한 중복 체크나 외래키 제약에 대한 체크 작업에는 여전히 갭락이나 넥스트키락이 사용될 것임 
...
- 서비스용으로 가장 많이 사용되는 형태
- 슬레이브 서버는 읽기전용으로 설정해서 마스터와 슬레이브의 데이터가 달라지지 않게 해줌
- 복제는 비동기 방식으로 마스터의 데이터가 슬레이브로 전달됨 따라서 바로 insert하고 select하는 쿼리는 마스터에서 실행해주는게 좋음
- 일반적으로 특별한 문제가 없다면 늦어도 1초미만의 시간에 슬레이브가 복제함



**1:M:M 복제**

- 1:M에서 슬레이브의 수가 너무 많아서 마스터에게 악영향이 끼칠것같으면 다음 구조를 고려해볼 수 있음
- 1차그룹은 oltp 서비스에, 2차 그룹은 통계 배치, 그리고 백업 용도로 구분해서 사용 가능함
- 이 복제 형태는 서버를 업그레이드하거나 장비를 일괄 교체할 때도 많이 사용됨



### 스케일 아웃

- 읽기를 확장하는것이지 쓰기작업을 확장하는 것은 아님
...
- 마스터가 깨지는경우는 거의없음
- 마스터가 만약에 깨진다면 슬레이브를 마스터로 승격시키면됨



### 복제가 구축된 mysql에서의 작업

- mysql 복제에서 마스터 에서 사용자가 동시다발적으로 실행한 sql이 슬레이브 mysql에서는 하나의 스레드에 의해 직렬화되어 하나씩 순차적으로 실행됨 그래서 시간이 오래 걸리는 인덱스나 칼럼 추가와 같은 작업은 마스터 mysql 서버에서만 실행하기보단 마스터와 슬레이브 각각의 mysql 서버에서 별도로 실행해주는것이 좋음
- 인덱스를 생성하는 작업도 마찬가지로 인덱스 생성땐 복제가 멈춤 따라서 개별적으로 생성해주는 것이 좋음
- SET sql_log_bin = OFF;로 복제를 멈출것
- 작업후에는 다시 ON으로 돌릴것
...
- 가능하다면 여러 테이블의 조인 여부와 상관없이 짧은 이름으로 테이블의 별칭을 부여하는 습관을 들일것

### 서버 사이드 프리페어 스테이트먼트 사용

- 5.0이하에서는 프리페어 스테이트먼트를 사용하면 MYSQL의 쿼리캐시를 사용할 수 없었음
- 5.1에서 개선 프리페어 스테이트먼트와 쿼리캐시 동시사용 가능
- 서버사이드 프리페이 스테이트먼트를 반드시 활성화시킬것



### FULL GROUP BY 사용

- FULL GOUP BY를 사용하는 습관



### DELETE UPDATE  쿼리에서 ORDER BY ..LIMIT .. 사용 자제

- 5.0까진 괜찮았는데 그 이후에는 복제에서 문제가 생길 수 있음
...
- UNION도 가급적 사용 자제하고 가급적 UNION ALL을 사용할 것

### 

### 스토어드 함수는 가능하면 DETERMINISTIC으로 정의

- 스토어드함수에게 DETERMINISTIC 키워드를 추가해서 그 함수나 프로시저가 입력 값이 똑같으면 출력 값도 같다는 것을 옵티마이저에게 알려주는게 좋음
- 인덱스와 관련 있음
- NO DETERMINISTIC이 DEFAULT

### 

### 스토어드 프로그램에서는 예외 처리 코드를 작성

- 스토어드 프로시저나 함수는 디버깅이 쉽지 않음
- 핸들러와 컨디션을 이용한 에러 핸들링 참고
...
- 프로그램 작성할땐 처리된 레코드 건수를 반드시 검증하는 형태의 프로그램 로직을 추가하는 습관을 들일 것



### 숫자 값은 반드시 숫자 타입의 칼럼으로 정의

- char_type_column=2 같은경우는 char_type_column의 모든 내용을 숫자로 변형해서 검색하는 풀테이블스캔이 나옴
- 순수한 숫자 값은 숫자 타입에 저장하고 알파벳이나 숫자가 혼용되는 값은 char나 varchar타입에 저장하자.
