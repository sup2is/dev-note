...
마이크로서비스 아키텍처는 복잡한 모놀리식 소프트웨어 시스템을 더 작고 다루기 쉬운 부분으로 분해하는 강력한 설계 패러다임이다. 하지만 이런 분산된 환경에서 디버깅은 매우 힘들어진다. 이런 분산 디버깅에 필요한 세가지 도구를 살펴본다.



1. **스프링 클라우드 슬루스:** 상관관계 ID를 사용해 HTTP 호출을 측정하는 스프링 클라우드 프로젝트이다. 생성 중인 추적 데이터를 오픈집킨에 공급할 수 있는 연결고리를 제공한다. 새엇ㅇ되는 상관관계 ID를 모든 시스템 호출로 통과시키기 위해 필터를 추가하고 다른 스프링 컴포넌트와 상호 작용하는 방식으로 슬루스를 활용한다.
2. **페이퍼트레일:** 여러 데이터 소스의 로그 데이터를 검색이 가능한 단일 데이터베이스로 수집하는 클라우드 기반의 프리미움 서비스(기본 서비스는 무료로 제공하고 추가 고급 기능에 대해서는 요금을 받는 형태)다. 로그 수집에 대해 사내 구축형과 클라우드기반 오픈소수 상용 솔루션 등 여러 대안이 있다.
3. **집킨:** 여러 서비스 사이의 트랜잭션 흐름을 보여주는 오픈소스 기반의 데이터 시각화 도구다. 집킨은 트랜잭션을 컴포넌트별로 분해하고 성능 과열점이 어디서 발생했는지 시각적으로 확인할 수 있다.



## 스프링 클라우드 슬루스와 상관관계ID

앞전에 스프링 클라우드 주울을 확인하면서 상관관계ID에 대해 알아봤다. 상관관계 ID는 UUID형태의 문자열이고 트랜잭션을 시작할때 주입된다.

스프링 클라우드 슬루스는 다음 역할을 한다
...
spring-cloud-starter-sleuth 모듈을 사용한다. 모듈만 추가하면 알아서 적용된다.

이 모듈은 다음 기능을 제공해준다.

1. 서비스로 들어오는 모든 HTTP호출을 검사하고 그 호출에서 스프링 클라우드 슬루스의 추적 정보가 존재하는지 확인한다. 추적 정보가 있다면 마이크로서비스로 전달된 추적 정보를 수집해 로깅 및 처리를 위해 서비스에 제공한다.
2. 스프링 클라우드 슬루스 추적 정보를 스프링 MDC에 추가해 마이크로서비스에서 생성된 모든 로그문이 로그에 추가된다.
...
대규모 마이크로서비스 환경에서 로그 데이터는 플랫폼을 디버깅하는 중요한 도구다. 하지만 세분화된 마이크로서비스의 로그 데이터를 한곳으로 연결하는 것은 상당히 어려운 일이다. 여러 서비스 사이의 문제를 디버깅하려는 개발자는 종종 다음 작업을 시도해야 한다.

- 여러 서버에 로그인하며 각 서버의 로그를 검사해야 한다. 특히 이 작업은 문제된 서비스의 트랜잭션 양이 달라 로그를 롤오버 하는 속도가 다를 때는 상당히 힘들다.
- 로그를 파싱하고 관련 로그 항목을 식별하는 자체 질의 스크립트를 작성해야 한다. 쿼리가 모두 다를 수 있어 로그에서 데이터를 질의하는 사용자 정의 스크립트가 엄청나게 늘어날 때가 많다.
- 개발자가 서버에 있는 로그를 백업해야 하므로 다운된 서비스 복구 과정을 연장해야한다. 서비스를 호스팅하는 서버가 완전히 고장난다면 대개 로그는 유실된다.



ELK, 그레이로그, 스플렁크, 수모로직, 페이퍼트레일 등등의 로깅 아키텍쳐가 존재한다.

페이퍼트레일의 장점

1. 무료형 계정으로 등록할 수 있는 프리미움 모델을 제공한다.
2. 설정이 매우 쉽다. 특히 도커같은 컨테이너 런타임과 설정하기 쉽다.
3. 클라우드 기반이다. 좋은 로깅 인프라스트럭처는 마이크로서비스 어플리케이션에 중요하다고 생각하지만, 대부분의 조직에서 로깅 플랫폼을 적절히 구축하고 관리할 시간이나 기술 인력이 없다고 생각된다.

### 

### 스프링 클라우드 슬루스와 페이퍼트레일의 실제 구현

**페이퍼 트레일을 설정하기 위한 작업**

1. 페이퍼트레일 계쩡을 생성하고 페이퍼트레일의 syslog 커넥터를 구성한다.
2. 모든 도커 컨테이너의 표준 출력을 포착하기 위해 로그스파우트 도커 컨테이너를 정의한다.
3. 스프링 클라우드 슬루스의 상관관계 ID를 기반으로 질의문을 실행해 구현 테스트한다.



**흐름**

1. 각 컨테이너는 모두 Docker.sock이라는 내부 파일 시스템에 표준 출력을 기록한다.
2. Docker.sock과 페이퍼트레일 사이에 로그 스파우트 도커 컨테이너를 두어 Docker.sock을 수신 대기하고 표준 출력으로 가는 모든 데이터를 원격 syslog 위치에 기록한다.
3. 페이퍼트레일은 특정 syslog 포트를 사용자의 어플리케이션에 제공하고 유입된느 로그 데이터를 받아 인덱싱해서 저장한다.
4. 사용자는 페이퍼트레일 웹 어플리케이션에서 질의를 수행한다. 스프링 클라우드 슬루스의 추적 id를 입력하고 해당 추적 id가 포함된 다른 서비스의 모든 로그를 볼 수 있다.



**로그스파우트의 특징(도커 로깅 드라이버를 사용하지 않은 이유)**

- **한번에 여러 엔드포인트로 로그 데이터 전송:** 많은 기업은 로그 데이터를 로그 수집 플랫폼으로 보내고 전달된 로그에서 중요한 데이터를 모니터링하는 보안 모니터링 도구로 보내길 원할 것이다.
- **로그 데이터를 전송할 컨테이너를 중앙에서 필터링:**도커 드라이버를 사용하면 docker-compose.yml 파일에서 각 컨테이너에 대한 로그 드라이버를 수동으로 설정해야 한다. 로그스파우트는 특정 컨테이너뿐만 아니라 특정 스트링 패턴도 필터를 중앙의 구성 파일에서 설정할 수 있다.
- **어플리케이션에서 로그 정보를 특정 http 엔드포인트로 기록하는 http 경로:** 이 기능을 사용하면 특정 로그 메시지를 특정 하위 로그 수집 플랫폼에 기록할 수 있다. 예를 들어 일반적인 stdout/stderr 로그 메시지는 페이퍼트레일로 보내고 특정 어플리케이션 audit 정보는 사내 엘라스틱서치 서버에 보낼 수 있다.
- syslog를 넘어 프로토콜의 통합: 로그스파우트를 사용하면 메시지를 udp와 tcp 프로토콜로 전송할 수 있다. 로그 스파우트는 도커의  stdout/stderr를 엘라스틱서치로 통합하는 타사 모듈도 지원한다.



**로깅 통합에 대하여**

1. **서비스 개발 초기에 로깅 전략을 정의하고 구현하라:** 로깅 인프라 구현은 지루하고 때로는 힘들며 프로젝트가 한참 진행된 후에는 시간이 오래 걸린다.
2. **로깅은 마이크로서비스 인프라스트럭처의 핵심 요소다:** 자체 로깅 솔루션을 구현하거나 사내 구축형 로깅 솔루션을 구현하기로 결정하기 전에 충분하고 신중히 고민하라. 클라우드 기반 로깅 플랫폼은 비용을 들일 가치가 있다.
3. **로깅 도구를 학습하라:**거의 모든 로깅 플랫폼에는 통합 로그에 질의할 때 쓰는 질의 언어가 있다. 로그는 정보와 지표에 대한 놀라운 원천이며, 본질적으로 또 다른 유형의 데이터베이스이므로 질의 학습에 시간을 투자하는 것은 큰도움이 될 것이다.
...
분산 추적은 여러 마이크로서비스 간에 트랜잭션이 어떻게 흐르고 있는지 시각적으로 보여준다. 집킨은 여러 서비스 호출 사이의 트랜재션을 추적하는 분산 추적 플랫폼이다. 집킨을 사용하면 트랜잭션의 소요 시간을 그래픽으로 확인할 수 있고, 호출에 관련된 각 마이크로서비스별로 소요된 시간을 분설할 수 있다. 

스프링 클라우드 슬루스와 집킨 설정은 네가지 작업을 포함한다.

- 추적 데이터를 기록할 서비스에 스프링 클라우드 슬루스와 집킨 jar 파일 추가
- 추적 데이터를 수집하는 집킨 서버에 연결하도록 서비스의 스프링 프로퍼티 구성
- 데이터를 수집하는 집킨 서버 설치 및 구성
- 각 클라이언트가 집킨에 추적 정보를 전송하는데 필요한 샘플링 전략 정의

스프링 클라우드와 집킨을 함께사용하기 위해서

spring-cloud-start-sleuth 모듈과 spring-cloud-sleuth-zipkin 모듈을 추가한다.
...
- 스프링 클라우드 슬루스를 사용하면 마이크로서비스 호출에 추적 정보를 완벽하게 추가할 수 있다.
- 상관관계 ID는 여러 서비스 사이에서 로그 항목을 연결하는데 사용된다. 이 ID로 한 트랜잭션에 연관된 여러 서비스에서 트랜잭션 동작을 확인할 수 있다.
- 상관관계 ID가 강력하지만, 다양한 출처에서 오는 로그를 수집하고 수집된 내용을 검색하며 질의할 수 있는 수집 플랫폼과 상관관계 ID 개념은 짝을 이루어야 한다.
- 여러가지 사내 구축형 로그 수집 플랫폼이 존재하지만, 클라우드 기반 서비스를 사용하면 대규모 인프라스트럭처를 갖추지 않고도 로그를 관리할 수 있다. 어플리케이션의 로깅 용량이 증가해도 쉽게 확장할 수 있다.
- 로그 수집 플랫폼과 도커 컨테이너를 통합해 컨테이너의 표준 출력 에러로 기록되는 모든 로그 데이터를 수집한다. 이 장에서는 도커 컨테이너를 로그스파우트와 클라우드 로깅 플랫폼 공급자인 페이퍼트레일과 통합해 로그를 수집하고 쿼리 할 수 있었다.
- 통합된 로깅 플랫폼이 중요하지만 마이크로서비스 사이의 트랜잭션을 시각적으로 추적할 수 있는 기능도 매우 소중한 도구이다.
- 집킨을 사용하면 서비스 호출이 이루어질 때 서비스 사이에 존재하는 의존성을 볼 수있다
- 스프링 클라우드 슬루스는 집킨과 통합된다. 집킨은 트랜잭션 흐름을 그래픽으로 보여주고 사용자 트랜잭션에 연관된 마이크로서비스의 성능 특성을 이해할 수 있게 한다.
- 스프링 클라우드 슬루스는 슬루스가 활성화 된 서비스에서 사용되는 http 호출과 인바운드/아웃바운드 메시지 채널에 대한 추적 데이터를 자동으로 포착한다. 
- 스프링 클라우드 슬루스는 각 서비스 호출을 스팬 개념에 매핑한다. 집킨에서 각 스팬의 성능을 볼 수 있다.
- 스프링 클라우드 슬루스와 집킨에서 스프링 기반이 아닌 데이터베이스의 성능을 파악하도록 사용자 정의 스팬을 재정의할 수 있다.
...
- **자동화:** 코드를 빌드할 때 빌드와 배포 프로세스에서 사람이 개입하지 않아야 한다. 특히 dev나 stage서버는 더더욱 그렇다.  모든 과정이 자동화되어야 하며 소스 저장소에 코드를 커밋하는 동작에서 시작되어야 한다.
- **반복성:** 소프트웨어를 빌드 배포하는 일은 반복 가능해야한다.
- **완전성:** 배포된 산출물의 결과는 서비스를 위한 완전한 런타임 환경을 포함하는 완비된 가상 머신이나 컨테이너 이미지여야 한다. 이것은 인프라를 생각하는 방식에서의 중요한 변화다. 
- **불변성:** 서비스를 포함한 머신 이미지를 빌드하고 배포한 후에 이미지에 대한 런타임 구성을 건드리거나 변경해서는 안된다. 변경이 필요하다면 형상관리 툴에 커밋하고 빌드를 빌드 배포를 반복해야한다. 



**마이크로서비스의 빌드 배포 파이프라인**

1. 개발자는 서비스 코드를 소스 저장소에 커밋한다.
2. 빌드 및 배포 엔진은 코드를 저장소에서 가져와 빌드 스크립트를 실행한다.
3. 엔진은 코드를 컴파일하고 테스트를 수행하고 실행 가능한 산출물로 생성한다.
4. 서비스와 런타임 엔진이 설치된 가상 머신 이미지가 생성된다.
5. 새로운 환경으로 승격되기 전에 머신 이미지에 대해 플랫폼 테스트를 수행한다.
6. 머신 이미지를 다음 환경으로 승격하려면 해당 환경에 대해 플랫폼 테스트를 수행하야 한다.



**단위테스트 vs 통합테스트 vs 플랫폼 테스트**

- **단위테스트:** 단위 테스트는 서비스 코드의 컴파일 직후, 환경에 배포하기 전에 수행된다. 각 단위의 단위 테스트는 크기가 작고 좁은 범위로 제한되며 완전히 격리해서 실행하도록 설계되었다. 단위 테스트는 제 3자 인프라 데이터베이스, 서비스 등에 의존성이 없어야 한다. 일반적으로 단위 테스트의 범위는 단위 메서드나 함수가 해당된다.
- **통합테스트:** 통합 테스트는 서비스 코드를 패키징한 직후에 수행된다. 이 테스트는 저체 워크플로를 테스트하고 해당 서버 박스에서 호출하는 주요 서비스나 컴포넌트를 스텁 또는 목을 사용하도록 설계되었다. 통합 테스트 동안 데이터를 보관하는데 인메모리 데이터베이스를 사용하거나 외부 서비스 호출에 목을 사용할 수 있다. 통합 테스트는 전체 워크플로와 코드 경로를 테스트하며 외부 의존성은 목이나 스텁을 사용하므로 원격 서비스에 대한 호출도 빌드 서버를 벗어나지 않는다.
- **플랫폼 테스트:** 플랫폼 테스트는 서비스가 해당 환경에 배포되기 직전에 수행된다. 이 테스트는 일반적으로 전체 비지니스 플로를 테스트하고 운영 환경 시스템에서 정상적으로 호출되는 외부 의존성도 모두 호출한다. 플랫폼 테스트는 특정 환경에서 실제 수행되고 목 서비스는 사용하지 않는다. 플랫폼 테스트는 보통 통합테스트 동안 스텁으로 대신한 외부서비스에서 검출되지 않은 통합 문제를 확인하기 위해 수행된다.



**빌드 및 배포 프로세스의 핵심 패턴**

- **지속적 통합/지속적 전달:** CI/CD를 사용하면 어플리케이션 코드가 커밋되면 코드가 빌드되고 테스트될 뿐만 아니라 지속적으로 배포된다. 코드 배포는 단위,통합,플랫폼 테스트를 통과하면 즉시 다음 환경으로 승격되도록 진행해야 한다. 대부분의 조직에서 유일하게 멈출 시점은 운영 환경에 적용할 때다.
- **코드형 인프라스트럭처:** 개발 환경 및 다른 환경에 적용될 마지막 소프트웨어 산출물은 머신 이미지다. 머신 이미지와 이미지에 설치된 마이크로서비스는 마이크로서비스 소스 코드를 컴파일하고 테스트한 직후 프로비저닝 된다. 머신 이미지 프로비저닝은 각 빌드와 함께 실행되는 일련의 스크립트를 통해 수행된다. 서버가 구축된 후에는 수동으로 수정되어서는 안된다. 프로비저닝 스크립트 소스 제어에 저장되어 일반 코드처럼 관리된다.
- **불변 서버:** 서버 이미지가 생성되면 서버 구성과 마이크로서비스를 프로비저닝 프로세스 이후에 절대 변경할 수 없다. 이는 개발자나 시스템 관리자가 나중에 장애의 원인이 될 수 있는 작은 변경에 의해 구성 편차를 겪지 않도록 해준다. 변경이 필요하면 서버 프로비저닝 스크립트를 변경하고 새 빌드를 시작해야 한다. 



**Travis CI를 사용한 빌드 배포 파이프라인**

1. 개발자가 깃헙에 있는 마이크로서비스 코드를 업데이트 한다.
2. Travis CI는 업데이트된 코드 저장소에서 가져와 travis.yml 파일을 사용해 빌드 및 배포 프로세스를 시작한다.
3. 빌드에서 사용되는 언어나 환경변수 등 기본 빌드 구성을 설정한다.
4. 빌드에 필요한 외부 라이ㅡ러리나 명령줄 도구를 설치한다.
5. 빌드 이름으로 저장소에 태그를 붙인다.
6. Travis는 메이븐 빌드 스크립트를 실행한다 (도커 이미지 생성)
7. 도커 이미지를 도커 허브에 올린다.
8. 서비스를 아마존 ECS에 올린다.
9. 플랫폼 테스트를 시작한다.
...
- 빌드 및 배포 파이프라인은 마이크로서비스 제공을 위해 매우 중요한 부분이다. 제대로 작동하는 빌드 및 배포 파이프라인을 통해 새로운 기능과 버그 수정을 몇 분 안에 배포할 수 있다.
- 빌드 및 배포 파이프라인은 서비스를 제공하는데 사람의 직접적인 개입 없이 자동화되어야 한다. 프로세서의 수동 부분은 변동 및 실패 가능성을 나타낸다.
- 빌드 및 배포 파이프라인의 자동화에는 많은 스크립팅과 올바른 구성이 필요하다. 자동화 구축을 위해 필요한 일의 양을 과소평가해서는 안된다.
- 빌드 및 배포 파이프라인은 불변 가상 머신 또는 컨테이너 이미지를 제공해야 한다. 서버 이미지가 생성된 후에는 절대 변경되면 안된다.
- 환경별 서버 구성은 서버가 설정될 때 매개 변수로 전달되어야 한다.
