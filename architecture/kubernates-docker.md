...
- 쿠버네티스 등장 배경
  ...
  - 구글이 자사 컨테이너 운영 플랫폼으로 운영하던 보그(Borg)를 기반으로하는 쿠버네티스를 오픈소스로 공개

## 쿠버네티스 이해하기

- 쿠버네티스는 컨테이너 오케스트레이션 솔루션이다
...
- kubectl get pods --all-namespaces
  - default namespace외에 모든 것을 표시하겠다는 의미.
...
- ectd(2)
  ...
  - ectd의 정보만 백업되어 있다면 쿠버네티스 클러스터를 복구할 수 있다.
  - ectd는 분산 저장이 가능한  key-value 저장소이므로 이중화시켜놓으면 시스템의 가용성을 확보할 수 있다 =>  kubespray
- 컨트롤러 매니저(3)
...
- 파드(7)
  - 한 개 이상의 컨테이너로 단일 목적의 일을 하기 위해서 몽니 단위
  - 파드는 **언제라도 죽을 수 있는 존재**
