...
- 실행자, 동시성 컬렉션, 동기화 장치 <- 고수준 유틸리티의 세가지 범주
- 동시성 컬렉션은 List Queue Map같은 표준 인터페이스에 동시성을 가미해 구현한 고성능 컬렉션, 동시성 컬렉션에서 동시성을 무력화하는것은 불가하고 외부에서 락을 추가로 사용하면 오히려 속도가 더 느려짐
- 이제는 Collections.synchronizedMap보다 ConcurrentHashMap을 사용하는게 훨씬 좋음
- wait 메서드를 사용할 때는 반드시 대기 반복문 관용구를 사용하라. 반복문 밖에서는 절대로 호출하지 말자.
- 이외에는 개어려움

**핵심**

- wait과 notify를 직접 사용하는 것을 동시성 '어셈블리 언어'로 프로그래밍 하는 것에 비유할 수 있음 반면 java.util.concurrent는 고수준 언어에 비유할 수 있음. 코드를 새로 작성한다면 wait과 notify를 쓸 이유가 거의 없음 이들을 사용하는 레거시 코드를 유지보수해야 한다면 wait는 항상 표준 관용구에 따라 while문 안에서 호출하도록 하자. 일반적으로 notify 보다는 notifyAll을 사용해야 한다. 혹시라도 notify를 사용한다면 응답 불가에 빠지지 않도록 각별히 주의하자
...
- 직렬화는 보안에 취약하기때문에 회파하는것이 가장 좋음
- 새로운 시스템에서 자바 직렬화를 써야할 이유는 전혀 없음
- 신뢰할 수 없는 데이터는 절대 역직렬화를 하지 않는다
