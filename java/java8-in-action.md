...
- 일반값처럼 취급할 수 있는 함수를 일급함수라함

### 고차원 함수



- 함수형 프로그래밍 커뮤니티에 따르면 Comparator.comparing처럼 다음 중 하나 이상의 동작을 수행하는 함수를 고차원 함수라 함
  1. 하나 이상의 함수를 인수로 받음
  2. 함수를 결과로 반환
- 자바 8에서는 함수를 인수로 전달할 수 있을 뿐 아니라 결과로 반환하고 지역 변수로 할당하거나 구조체로 삽입할 수 있으므로 자바 8의 함수도 고차원 함수라고 할 수 있음.



### 커링

- 커링은 x와 y라는 두 인수를 받는 함수 f를 한 개의 인수를 받는 g라느 ㄴ함수로 대체하는 기법임
...
- 함수형 프로그램에서는 함수형 자료구조, 불변자료구조 등의 용어로 사용하지만 보통 영속자료구조라고 부름
- 함수형 메서드에서는 전역 자료구조나 인수로 전달된 구조를 갱신 불가함 참조 투명성을 위배하고 우니수 결과로 단순하게 매핑할 수 있는 능력이 상실되기 때문
