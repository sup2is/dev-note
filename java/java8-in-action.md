...
- 자바9에서는 메서드 구현을 포함하는 인터페이스를 정의할 수 있음 결과적으로 기존 인터페이스를 구현하는 클래스는 자동으로 인터페이스에 추가된 새로운 메서드의 디폴트 메서드를 상속받게 됨 이런식으로 하면 기존 코드 구현을 바꾸도록 강요하지 않으면서도 인터페이스를 바꿀 수 있음
- 디폴트 메서드는 주로 라이브러리 설계자들이 사용함
- 추상클래스와 여전히 차이점은 존재
- 기존에 공개된 인터페이스에 메서드를 추가할때 디폴트 메서드를 제공함으로써 실제 클라이언트들이 해당 메서드를 구현하지 않아도 문제 없도록 함



## 디폴트 메서드란 무엇인가?

- 메서드 앞에 default라는 키워드로 시작함
- 추상메서드지만 메서드 바디를 포함함
- 따라서 인터페이스에 메서드가 추가되더라도 확장 가능한 형태가됨
- List의 sort, Collection의 stream 메서드는 자바8의 default로 추가되었음



## 디폴트 메서드 활용 패턴

선택형 메서드와 동작 다중 상속이 가능함

### 선택형 메서드

- 기존에는 필요하지 않은 메서드도 구현했지만 디폴트 메서드를 사용하면 기본 메서드 구현을 제공할 수 있으므로 불필요한 코드의 양을 줄일 수 있음

### 동작 다중 상속

- 디폴트 메서드로 기존에는 불가능했던 동작 다중 상속 기능도 구현 가능함
- 디폴트 메서드를 사용하지 않아도 기존에 자바는 다중 구현을 지원했음
...
1. 클래스가 항상 이긴다 클래스나 슈퍼 클래스에서 정의한 메서드가 디폴트 메서드보다 우선권을 가짐
2. 1번 규칙 이외의 상황에서는 서브인터페이스가 이김. 상속관계를 갖는 인터페이스에서 같은 시그니처를 갖는 메서드를 정의할 때는 서브인터페이스가 이김. 즉 B가 A를 상속받는다면 B가 A를 이김
3. 여전히 티폴트 메서드의 우선순위가 결정되지 않았다면 여러 인터페이스를 상속받는 클래스가 명시적으로 디폴트 메서드를 오버라이드하고 호출해야함
...
- 함수형 프로그래밍이 제공하는 부작용 없음과 불변성이라는 개념이 개발자 문제 해결에 도움을 줌

### 공유된 가변 데이터

- 공유 가변 데이터 구조를 사용하면 프로그램 전체에서 데이터 갱신 사실을 추적하기가 어려워짐
- 자신을 포함하는 클래스의 상태 그리고 다른 객체의 상태를 바꾸지 않으며 return문을 통해서만 자신의 결과를 반환하는 메서드를 순수 메서드 또는 부작용 없는 메서드라고 함
- 불변 객체를 이용해서 부작용을 없앨 수 있음 불변 객체는 인스턴스화한 다음에는 객체의 상태를 바꿀 수 없는 객체이므로 함수 동작에 영향을 받지 않음
- 따라서 불변 객체는 복사하지 않고 공유할 수 있고, 객체의 상태를 바꿀 수 없으므로 스레드 안전성을 제공함

### 선언형 프로그래밍

- 프로그램으로 시스템을 구현하는 방식은 크게 두가지로 구분 가능
- '이일을 먼저하고 .. 그 다음에 저 값을 갱신하고 .. 그 다음에' 처럼 작업을 어떻게 수행할 것인지에 집중하는 방법이  있음
- 어떻게 how에 집중하는 프로그래밍 형식은 고전의 객체지향 프로그래밍에서 이용하는 방식임 (명령형 프로그래밍)
- 무엇을 what에 집중하는 방식도 있음 이것을 선언형 프로그래밍이라고함 ex: 스트림의 내부반복
- 선언형 프로그래밍에서는 우리가 원하는 것이 무엇이고 시스템이 어떻게 그 목표를 달성할 것인지 등의 규칙을 정함. 문제 자체가 코드로 명확하게 드러나는 점이 선언형 프로그래밍의 강점임
...
- 부작용이 없는 계산을 지향함 선언형 프로그래밍과 부작용을 멀리한다는 두가지 개념은 좀 더 쉽게 시스템을 구현하고 유지보수하는데 도움을 줌



## 함수형 프로그래밍이란 무엇인가?

- 함수형 프로그래밍에서 함수란 수학적인 함수와 같다. 즉 함수는 0개이상의 인수를 가지며 한 개 이상의 결과를 반환하지만 부작용이 없어야함

> 함수형이라는 말은 '수학의 함수처럼 부작용이 없는'을 의미함. 결과적으로 함수형 프로그래밍은 지금까지의 프로그래밍과는 사뭇 다른 모습일 것임. 함수형 프로그래밍에서는 함수 그리고 if-then-else 등의 수학적 표현만 사용해야하는가? 아니면 시스템의 다른 부분에 영향을 미치지 않는다면 내부적으로는 함수형이 아닌 기능도 사용할 수 있을까? 즉, 내부적으론느 부작용이 발생하지만 호출자가 이를 알아차리지 못한다면 실제로 부작용이 발생한 것이라고 말할 수 있을가? 호출자에 아무 여향을 미치지 않는다면 호출자는 내부적인 부작용을 파악하거나 신경 쓸 필요가 없다. 결론적으로 '함수 그리고 if-then-lse 등의 수학적 표현만 사용'하는 방식은 순수 함수형 프로그래밍이라고 하며 '시스템의 다른 부분에 영향을 미치지 않는다면 내부적으로는 함수형이 아닌 기능도 사용' 하는 방식을 함수형 프로그래밍이라 한다.
...
- 자바에서는 순수 함수형이 아니라 함수평 프로그램을 구현할 것임 실제 부작용이 있지만 아무도 이를 보지 못하게 함으로써 함수형을 달성할 수 있음
- 부작용을 일으키지 않는 어떤 함수나 메서드가 있는데 다만 진입할 때 어떤 필드의 값을 증가시켰다가 빠져나올 때 필드의 값을 돌려놓는다고 가정하면 단일 스레드로 실해오디는 프로그램의 입장에서는 아무런 부작용도 없기 때문에 함수형이지만 멀티스레드에서 다른 스레드의 값을 확인할때는 부작용이 있기 때문에 함수형이 아님 이럴 경우 메서드 바디를 잠금으로써 이 문제를 해결할 수 있고 이때는 함수형임 하지만 병렬로 실행 불가능하기때문에 프로그래머 관점에서는 프로그램의 실행 속도가 느려지는 것!
- 함수나 메서드는 직연 변수만을 변경해야 함수형이라 할 수 있음
- 함수나 메서드에서 참조하는 객체가 있다면 그 객체는 불변 객체여야함
- 예외적으로 메서드 내에서 생성한 객체의 필드는 갱신할 수 있음, 단 새로 새엇ㅇ한 객체의 필드 갱신이 외부에 노출되지 않아야하고 다음에 메서드를 다시 호출한 결과에 영향을 주면 안됨
- 함수형이라면 함수나 메서드가 어떤 예외도 일으키지 않아야함
- 예외를 사용하지 않고 나눗셈 같은 함수를 표현하려면 Optinal을 사용할것 하지만 적절하게 사용하는게 중요함 다른 컴포넌트에 영향을 미치지 않고 지역적으로만 예외를 사용하는 방법을 고려할 수 있음
- 함수형에서는 비함수형 동작을 감출 수 있는 상황에서만 부작용을 포함하는 라이브러리 함수를 사용해야함(즉 먼저 자료구조를 복사한다는다 발생할 수 있는 예제를 적절하게 내부적으로 처리함으로써 자료구조의 변경을 호출자가 알 수 없게 감춰야함)



### 참조 투명성

- 부작용을 감춰야 한다 라는 제약은 참조 투명성 개념으로 귀결됨 즉 같은 인수로 함수를 호출했을때 항상 같은 결과를 반환한다면 참조적으로 투명한 함수라고 표현함
- String.replace는 항상 같은 결과가 나오므로 참조적으로 투명하지만 Random.nextInt는 참조 투명성을 위배함
- 참조 투명성은 비싸거나 오래걸리는 연산을 기억화 또는 캐싱을 통해 다시 계산하지 않ㄱ 저장하는 최적화 기능도 제공함
...
- 결론적으로 자바 8에서는 반복을 스트림으로 대체해서 변화를 피할 수 있음 반복을 재귀로 바꾸면 더 간결하고 부작용이 없는 알고리즘을 만들 수 있음
- 실제로 재귀를 이용하면 좀 더 쉽게 읽고, 쓰고, 이해할 수 있느 ㄴ예제를 만들 수 있음
...
- 같은 인수값으로 함수를 호출했을 때 항상 같은 값을 반호나하면 참조 투명성을 갖느 함수다. while 루프 같은 반복문은 재귀로 대체할 수 있다.
- 자바에서는 고전 방식의 재귀보다는 꼬리 재귀를  사용해아 추가적인 컴파일러 최적화를 기대할 수 있다.
