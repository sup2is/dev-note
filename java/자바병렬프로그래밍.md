...
- 가장 간단한 방법은 HTML을 순차적으로 렌덩링하는 방법이지만 사용자 입장에서는 느리게 느낄 수 있음
- 순차적인 방법은 cpu의 능력을 제대로 활용하지 못하는 경우가 많음
...
- Executor 프레임웍에서는 작업을 표현하는 방법으로 Runnable 인터페이스를 사용함
- 결과를 얻는데 시간이 걸리는 기능은 Runnable대신 Callable을 사용하는게 모양새가 좋음
- Callable인터페이스는 핵심 메서드인 call을 실행하고 나면 결과 값을 돌려받을 수 있고 Exception도 발생할 수 있도록 되어있음
- Executor에서 실행한 작업은 생성, 등록, 실행, 종료와 같은 네가지의 상태를 통과함
- Executor프레임웍에서는 먼저 등록됐지만 시작되지 않은 작업은 언제든지 실행하지 않도록 취소시킬 수 있음
- Future는 특정 작업이 정상적으로 완료됐는지, 아니면 취소됐는지 등에 대한 정보를 확인할 수 있도록 만들어진 클래스임
- Future가 동작하는 사이클에서 염두에 둬야 할 점은, 한 번 지나간 상태는 되돌릴 수 없다는 점.
- ExecutorService클래스의 submit메서드는 모두 Future 인스턴스를 리턴함. 따라서 Executor에 Runnable이나 Callable을 등록하면 Future인스턴스를 받을 수 있고 받은 Future인스턴스를 사용해 작업의 결과를 확인하거나 실행 도중에 작업을 취소할 수 있음



### 예제: Future를 사용해 페이지 렌더링

- html페이지 렌더링 프로그램의 병렬성을 높여 동작하도록 만들기위해 텍스트를 이미지로 그려내는 작업, html 페이지에서 사용한 이미지 파일을 다운받는 작업으로 두개로 나눔
...
- 이미지를 먼저 다운받는 기능의 Callable을만들어스 ExecutorService에 동록시키고 Callable이 등록되는 즉시 해당하는 작업에 대한 Future를 얻어냄 그리고 메인 작업이 실해오디는 과정에서 이미지 작업을 표현해야 하는 시점이 되면 Future.get 메서드를 호출해 해당하는 이미지 파일을 확보. 이미지를 다운받는 기능이 이미 모두 시작된 상태이기 때문에 순차적인 방법보다 효율적임
- 위 예제는 병렬로 프로그램을 처리해서 조금 더 빠르게 할 수 있으나 여전히 남은 문제점으로는 사용자는 모든 이미지의 다운로드가 완료된 후에 확인할 수 있다는것. 완료된 이미지 먼저 보여주는게 가장 이상적

### 다양한 형태의 작업을 병렬로 처리하는 경우의 단점

- 작업 A, B가 있는 전체 작업 AB가 있을때 A는 시간이 1000이 걸리고 B는 10이 걸린다고 가정하면 AB를 만드는데 총 소요되는 시간은 A + B임. 따라서 프로그램의 구조만 복잡해지고 순차실행과 비교해서 별다른 이점이 없음
- 프로그램이 해야 할 일을 작은 작업으로 쪼개 실행할 때 실제적인 성능상의 이점을 얻으려면, 프로그램이 하는 일을 대량의 동일한 작업으로 재정의해 병렬로 처리할 수 있어야함

### CompletionService: Executor와 BlockingQueue의 연합

- 처리해야 할 작업을 갖고 있고, 이 작업을 모두 Executor에 등록해 실행시킨 다음 각 작업에서 결과가 나오는 죽시 그 값을 가져다 사용하고자 한다면, 등록한 각 작업별로 Future객체를 정리해두고 타임아웃에 0을 지정해 get 메서드를 호출하면서 결과가 나왔는지 폴링해 결과를 찾아올 수 있음 <= 이런 작업을 위해 미리 만들어져 있는 방법이 바로 CompletionService임
- CompletionService는 Executor의 기능과 BlockingQueue의 기능을 하나로 모은 인터페이스임. 필요한 Callable작업을 등록해 실행시킬 수 있고. take나 poll과 같은 큐 메서드를 사용해 작업이 완료되는 순간 완료된 작업의 Future인스턴스를 받아올 수 있음



### 예제: CompletionService를 활용한 페이지 렌더링
...
- 이전과 다른점은 이미지를 받아오는 부분을 병렬로 실행한다는 것
- 이런 방법으로 사용하다 보면 Future가 단일 작업 하나에 대한 진행 과정을 고나리한다고 할 때, CompletionService는 특정한 배치 작업을 관리하는 모습을 띤다고 볼 수 있음



### 작업 실행 시간 제한

- 때로 작업이 너무 오래걸려서 더 이상 작업의 의미가 없는 경우도 있음
- 타임아웃을 지정할 수 있는 Future.get을 사용한다면 시간 제한과 관련된 요구사항을 만족할 수 있음
- 시간 제한과 관련된 요구사항과 더불어 더이상 필요없는 작업은 시스템의 자원을 소모하지 않도록 해야함
- Future에서 TimeoutException이 발생하면 작업을 취소하도록 할 수 있음
...
- 업체별로 입찰 정보를 가져오는 작업은 업체를 단위로 완전히 독립적인 작업임. 따라서 단일 입찰 정조를 가져오는 일이 작업의 단위로써 적절하다고 볼 수 있고, 입찰 정보를 가져오는 작업을 병렬로 처리할 수 있음. 입찰 정보를 가져오는 작업 n개를 생성해 스레드 풀에 등록하고, 등록한 작업마다 Future객체를 호가보하고, 타임아웃을 지정한 get 메서드로 각각의 입찰 정보를 가져오도록 할 수 있음. 이런 작업은 invokeAll 메서드를 사용하면 됨
- invokeAll은 지정된 시간이 지날때까지 대기하다가 리턴되는데 제한시간이 지날때까지 실행중이던 작업은 모두 실행이 취소됨
- invokeAll이 리턴되면 등록된 모든 작업은 완료되어 결과 값을 가지고 있거나 취소되거나 두가지 상태임



## 요약

- 애플리케이션을 작업이라는 단위로 구분해 실행할 수 있도록 구조를 잡으면 개발 과정을 간소화하고 병렬성을 확보해 병렬성을 높일 수 있음
- Executor 프레임웍을 사용하면 작업을 생성하는 부분과 작업을 실행하는 부분을 분리해 실행 정책을 수립할 수 있고 원하는 형태의 실행 정책을 쉽게 만들어 사용할 수 있음
- 작업을 처리하는 부분에서 스레드를 생성하도록 되어 있다면, 스레드를 직접 사용하는 대신 Executor를 사용해볼 것
- 애플리케이션이 하는 일을 개별 작업으로 구분해 처리할 때는 작업의 범위를 적절하게 잡아야함.
