...
- 스레드가 많아지면 많아질수록 ConcurrentHashMap의 성능이 극대화됨



## 컨텍스트 스위치 부하 줄이기

- 다양한 종류의 연산이 대기 상태에 들어갈 수밖에 없는 특성을 갖고 있는데 실행과 대기의 두 가지 상태를 옮겨 다니는 것을 컨텍스트 스위치라고 함
- 요청에 대한 서비스 시간이 늘어나는 일은 여러 가지 측면에서 좋지 않은 상태임
- 락을 확보하는 시간이 길수록 락에 대한 경쟁이 발생할 가능성이 높아짐
- 요청을 처리하는 스레드의 외부로 I/O 작업을 뽑아내는 방법은 요청을 처리하는 평균 시간을 줄여주는 좋은 방법임 ex log



## 요약

- 멀티스레드를 사용하는 큰 이유 증의 하나가 바로 다중 cpu 하드웨어를 충분히 활용하고자 하는 것.
- 병렬 처리 애플리케이션의 성능에 대해 논의하면서 실제적인 서비스 시간 보다는 애플리케이션의 데이터 처리량이나 확장성을 좀 더 집중적으로 살펴 봤었음
- 암달의 밥칙에 따르면 애플리케이션의 확장성은 반드시 순차적으로 실행돼야만 하는 코드가 전체에서 얼마만큼의 비율을 차지하냐에 달렸다고 함.
- 자바 프로그램의 내부에서 순차적으로 처리해야만 하는 가장 주요한 부분은 바로 독점적인 락을 사용하는 부분이기 때문에 락으로 동기화하는 범위를 세분화해 정밀도를 높이거나 락을 확보하는 시간을 최도한으로 줄이는 기법을 사용해 락을 최소한만 사용해야함.
- 독점적인 락 대신 독점적이지 않은 방법을 사용하거나 대기 상태에 들어가지 않게 하는것도 중요함
...
- 순차적인 프로그램에 비해 각 부분 간의 상호 작용이 훨씬 복잡하고 미리 예상하고 부넛ㄱ해야 할 가능한 오류 상황도 훨씬 많음
- 병렬 프로그램을 테스트한 결과는 안정성과 활동성으로 귀결됨
- 성능 측정
  - 처리량: 병렬로 실행되는 여러 개의 작업이 각자 할 일을 끝내는 속도
  - 응답성: 용청이 들어온 이후 작업을 마치고 결과를줄 때까지의 시간. 지연 시간이라고도 함
...
- 병렬 프로그램 테스트는 순차적인 프로그램을 테스트하는 경우와 똑같은 분석 작업으로 시작함
...
- take메서드는 avilableItems 세마포어에서 가져갈 항목이 있는지에 대한 확인을 받아야함. 만약 하나 이상이 있다면 즉시 확인에 성공. 아닐경우 항목이 추가될 때까지 대기함
- put 메서드는 take 메서드와 반대로 구성되어 있음
- 실제로 크기가 제한된 버퍼를 구현해 사용하고자 한다면 ArrayBlockingQueue 클래스나 LinkedBlockingQueue클래스를 사용하는게 올바른 방법



### 가장 기본적인 단위 테스트

- 위 SemaphoreBoundedBuffer는 자신이 갖고있는 데이터에 대한 정확한 테스트가 되어야함 
...
- 위 테스트는 순차적으로 동작하는 상황을 테스트함
- 이런 순차적인 테스트를 작성해두면 나중에 어떤 에러가 발생해도 멀티스레드 환경에서만 발생하는 오류가 아닌지 쉽게 알 수 있음
...
=======
- 대기 상태에 들어가는 메서드를 테스트하는 방법은 바로 인터럽트를 거는 방법이 있음. 예를 들어 대기상태에 들어가야 하는 메서드를 호출할 때는 따로 스레드를 실행시켜 호출하고, 해당 스레드가 대기 상태에 들어갈 때까지 기다리고 있다가 대기상태에 들어가면 인터럽트를 걸고, 원하는 연산을 제대로 처리햇는지 확인하는 순서로 진행함
...
- 병렬 처리 환경에서 동작하는 클래스의 기능을 동시 다발적으로 호출할 때 발생하는 문제를 제대로 테스트하려면, put 메서드나 take 메서드를 호출하는 여러 개의 스레드를 충분한 시간 동안 동작시킨 다음 테스트 대상 클래스의 상태가 올바른지, 잘못된 값이 들어있지는 않은지 확인해야함
- 안전성을 테스트하는 프로그램을 효과적으로 작성하려면 뭔가 문제가 발생했을 때 잘못 사용되는 속성을 높은 확률로 찾아내는 작업을 해야 함과 동시에 오류를 확인하는 코드가 테스트 대상의 병렬성을 인위로 제한해서는 안된다는 점을 고려해야함. 테스트 하는 대상 속성의 값을 확인할 때 추가적인 동기화 작업을 하지 않아도 된다면 가장 좋은 상태라고 볼 수 있음 <= 어렵
- 프로듀서-컨슈머 패턴을 사용해 동작하는 클래스에 가장 적합한 방법은 큐나 버퍼에 추가된 항목을 모두 그대로 뽑아 낼 수 있는지 확인하고 그 외에는 아무런 일도 하지 않는지 확인하는 방법 또 드란 방법으론 체크썸을 구한 다음 순서를 유지하는 세크섬 형태로 관리하고 쌓인 체크섬을 비교해 확인하는 방법이 있음 이 과정에서 체크썸을 구하는 부분을 동기화하느라 병목이 나오면 안됨
- 플랫폼마다 다르지만 스레드를 생성하고 실행하는 일이 상당히 부하가 걸리는 작업이고 이 작업이 매우 빠르게 끝난다면 병렬이아니라 순차적으로 실행될 가능성이 있음 이런 경우엔 CountDownLatch를 사용해 모든 스레드가 준비될 때까지 대기하고 또 다른 CountDownLatch를 사용해 모든 스레드가 오나료할 때까지 대기하는 방법이 있음 CyclicBarrier를 사용해도 같은 효과 발생 가능
- 테스트프로그램은 스레드가 교차 실해오디는 경우의 수를 최대한 많이 확보할 수 있도록 cpu가 여러 개 장착된 시스템에서 돌려보는게 좋음
...
- 미리 지정된 개수만큼의 연산을 실행하고 테스트를 마치는 프로그램은 버그로 인해 무제한으로 동작할 수 있는데 이럴땐 제한시간을 두는것도 좋은 방법



### 자원 관리 테스트

- 하지말아야 할 일을 정말 하지 않는지도 테스트해야함 ex 갖고 있던 객체의 참조를 해제하지 않는 행위
- 메모리 상황을 살펴볼 수 있는 힙 조사용 도구를 사용해볼만 함 힙덤프?
...
- testLeak() 메서드에서 heapSize1, 2의 차이가 거의 없어야함



### 콜백 사용

- 클라이언트가 제공하는 코드에 콜백 구조를 적용하면 테스트 케이스를 구현하는 데 도움이 됨
...
- 스레드 풀의 스레드 개수가 제대로 늘어나는지를 확인할 수 있는 테스트케이스



### 스레드 교차 실행량 확대

- 스레드 교차 실행 정도를 크게 높이고 그와 동시에 테스트할 대상 공간을 크게 확대시킬 수 있는 트릭이 있는데 바로 공유된 자원을 사용하는 부분에서 Thread.yield 메소드를 호출해 컨텍스트 스위치가 많이 발생하도록 유도할 수 있음
- yield() 메소드를 호출한 스레드는 실행 대기 상태로 돌아가고 동일한 우선순위 또는 높은 우선순위를 갖는 다른 스레드가 실행 기회를 가질 수 있도록 함
>>>>>>> da5558d725cbb315374ed3eb84eb9b006e6faaea



## 성능 테스트

- 성능 테스트 프로그램은 대부분 기능 테스트의 확장된 버전인  경우가 많음
- 가장 이상적인 시나리오는 테스트하고자 하는 대상 클래스가 실제 애플리케이션에서 사용되는 환경을 최대한 동일하게 반영해야함
- 성능 테스트는 성능과 관련된 스레드의 개수, 버퍼의 크기 등과 같은 각종 수치를 뽑아내고자 함



### PutTakeTest에 시간 측정 부분 추가

- 베리어 기반의 타이머를 사용하도록 변경한 test 메서드
...
- 프로듀서와 컨슈머 간에 데이터를 얼마나 빠르게 넘겨줄 수 있느냐 하는 수치를 얻을 수 있음 그리고 스레드의 개수가 많아질 때 크기가 제한된 버퍼가 얼마나 확장성을 받쳐주는지도 알아볼 수 있음



### 다양한 알고리즘 비교

- 연결리스트 기반의 큐는 put과 take 연산에 대해서 배열 기반의 큐보다 병렬 처리 환경에서 훨씬 안정적으로 동작함



### 응답성 측정

- 서비스 시간에 대한 분산을 시각적으로 표현할 수 있는 가장 효과적인 방법은 바로 작업을 처리하는 데 걸린 시간을 히스토그램으로 그려보는 방법
- 스레드가 아주 빡빡한 동기화 요구사항 때문에 계속해서 대기 상태에 들어가는 상황이 아니라면 불공정한 세마포어를 사용해 처리 속도를 크게 높을 수 있고 반대로 공정한 세마포어를 사용해 처리 시간의 분산을 낮출 수 있음
- 공정성 문제로 속도가 빨라지거나 분산 값이 줄어드는 정도가 굉장히 심한 편이기 때문에 세마포어를 사용할 때는 항상 어느 방법을 사용할 것인지 결정해야만 함



## 성능 측정의 함정 피하기

- 이론적으론 쉽지만 실제로 테스트를 작성할때는 잘못된 코딩 방법으로 작성하지 않도록 주의해야함

### 가비지 컬렉션

- 가비지 컬렉션이 언제 실행될 것인지를 미리 알고 있을 수가 없고, 시간을 측정하는 테스트 프로그램이 동작하는 동안 가비지 컬렉션 작업이 진행될 가능성도 높음
- 방법으로 가비지컬렉션의 실행을 멈춰두거나 가비지 컬렉션이 여러번 실행된다는 사실을 명확히 하고 테스트 결과에 가비지 컬렉션 부분을 적절하게 반영하도록 하는 방법이 있음 보통 후자를 많이 선택하는데 테스트 프로그램을 적당히 오랜 시간동안 동작시키면 실제와 유사한 결과를 얻어낼 수 있음

### 동적 컴파일

- 컴파일의 정도에따라 테스트 성능이 바뀔 수 있음
- 컴파일된 코드와 컴파일되지 않은 코드 때문에 성능 측정치가 올바르지 않을 경우 테스트 프로그램을 긴 시간동안 실행시켜 컴파일될 부분은 모두 컴파일되고 추가로 컴파일하거나 인터프리터로 실행되는 코드를 최소화하는 방법이 있음 또 다른 방법으로 시간을 측정하지 않는 워밍업 테스트를 실행시켜서 컴파일 시켜놓는것
...
- 병렬 테스트 프로그램에서 실제 상황과 유사한 결과를 얻어려면 직접적으로 알고자 하는 부분, 즉 병렬 처리 작업을 조율하는 동기화 부분의 성능과 함께 스레드 내부에서 실행되는 작업의 형태도 실제 애플리케이션과 비슷한 특성을 띠고 있어야함



### 의미없는 코드 제거

- 훌륭한 성능 측정 프로그램을 작성하려면 최적화 컴파일러가 의미 없는 코드를 제거하는 과정에 성능 측정상 필요한 부분까지 제거하지 않도록 약간의 편법을 써야 할 필요가 있음. 그러려면 프로그램 코드가 만들어내는 모든 결과 값을 프로그램 어디에선가 사용도록 해야함. 물론 그 때문에 추가적인 동기화를 해야 하거나 더 많은 자원을 소모하도록 하지 않는 것이 좋음
- I/O기능을 호출하면 성능 측정 값에 영향을 줄 수 있기 때문에 특히 시간을 측정하는 동안에는 I/O 기능을 사용하지 않는 편이 좋음



## 보조적인 테스트 방법

- 테스팅의 목적은 오류를 찾는 일이 아니라 대상 프로그램이 처음 작성할 때 설계했던 대로 동작한다는 신뢰성을 높이는 작업 이라고 봐야 함



### 코드 리뷰

- 단위 테스트와 성능 테스트만큼이나 중요하고 효과적인 방법은 여러명이 코드리뷰를 하는것



### 정적 분석 도구

- 정적 코드 분석 방법은 코드를 실행하지 않고 그 자체로 분석함 흔한 버그 패턴 등등 코딩오류를 발견할 수 있음

**일관적이지 않은 동기화**

- 다수의 클래스는 클래스 내부의 변수를 자신의 암묵적인 락으로 동기화한다는 동기화 정책을 많이 사용함
- 책에서 사용한 @GuardedBy와 같은 애너테이션이 표준화된다면 정적분석에서 사용할 수 있는듯..? <= 이게 맞는지 모르겠음



**Thread.run 호출**

- 일반적으로 Thread.start 메서드를 호출하는 대신 Thread.run 메서드를 호출하는 일은 잘못된 방법인 경우가 많음



**해제되지 않은 락**

- 암묵적인 락 대신 명시적인 락은 해당 락을 확보한 블록이 실행을 마치고 빠져나갔다해도 락이 자동으로 해제되지 않음. 표준적으로 락을 finally 구문에서 해제하는 방법이지만 그렇지 않은 경우 실행 도중에 예외가 발생했을때 락이 해제되지 않을 가능성이 있음

**빈 synchronized 블록**

- 이게 의미가 있을 수 있지만 대부분 잘못된 방법



**더블 체크 락**

- 더블 체크 락은 늦은 초기화 방법에서 발생하는 동기화 부하를 줄이기 위한 방법임
- 동기화 능력이 부족하기 때문에 공유된 변경 가능한 값을 읽어가는 등의 경우가 발생할 가능성이 있음



**생성 메서드에서 스레드 실행**

- 생성 메서드에서 새로운 스레드를 실행시키도록 한다면 해당 클래스를 상송받았을 땜 ㅜㄴ제가 생길 수 있고 this 변수를 스레드에게 노출시킬 수 있다는 위험도 있음



**알림 오류**

- notify나 notifyAll 메서드는 해당하는 조건 큐에서 대기하고 있는 스레드가 있다면 객체의 상태가 변경돼 대기중인 스레드가 대기 상태에서 v풀려나도 좋다는 것을 알려주는 메서드
- notify와 notifyAll 메서드는 항상 해당 조건과 관련된 상태가 변경됐을 때만 사용해야함

**조건부 대기 오류**

- 조건 큐에서 대기할 때는 필요한 락을 확보한 상태에서 상태 변수를 확인한 이후에 Object.wait이나 Condition.await 메서드를 반복문으로 감싸는 구조로 구현해야함



**Lock과 Condition의 오용**

- synchronized 블록에 락 인자를 넣을 때 Lock 이라는 클래스 이름을 지정한다거나 아니면 Condition.await 메서드를 호출하는 대신 Condition.wait 메서드를 호출하는 경우는 오타로 인해 동기화 구문을 잘못 작성하는 예



**락을 확보하고 대기 상태 진입**

- 락을 확보한 상태에서 Thread.sleep 메서드를 호출하면 락을 필요로하는 다른 스레드 ㅇ겨시 아무 일도 못하고 멍하니 대기 상태에 들어가게 할 수 있음

**스핀 반복문**

- 아무런 일도 하지 않으면서 특정 변수의 값이 원하는 상태에 도달할 때까지 계속해서 반복하기만 하는 반복문을 사용하면 cpu 자원을 엄청나게 소모할 뿐만 아니라 해당 변수가 volatile이 아니라면 심지어는 무한 반복에 빠질 수도 있음
- 원하는 형태로 상태 변수 값이 변경되기를 기다리는 경우에는 latch나 여러 가지 조건부 대기 기능을 활용하는 편이 안전



### 관점 지향 테스트 방법

- aop를 사용하면 상태 변수의 값이 동기화 정책에 잘 맞는지를 확인하는 등의 작업을 하도록 적용해볼 수 있음
...
- 병렬 프로그램에서 발생하는 오류는 대부분 발생확률이 아주 작은 타이밍 문제, 부하문제, 아니면 기타 쉽게 발현되지 않는 여러 원인에 의해 발생하기 때문임
- 테스트하는 대상에는 문제가 있었지만 테스트 프로그램에서 발생하는 동기화문제나 타이밍 문제 때문에 원래 테스트 대상이 갖고 있던 문제를 발견하지 못할 수도 있음
- 병렬 프로그램이 충분한 성능을 내는지 테스트하는 작업도 굉장히 어려움
- 정적으로 컴파일되는 c와 같은 언어에 비해 동적으로 컴파일되는 자바 언어로 작성된 병렬 처리 프로그램은 성능을 측정하기가 더 어려움
- 성능을 측정할 때의 실행 시간이 동적인 컴파일, 가비지 컬렉션, 각종 최적화 방법때문에 크게 변경될 수 있음
- 숨어 있는 버그를 상용 서비스 이전에 발견할 수 있는 가장 좋은 방법은 바로 전통적인 테스트 방법과 함께 코드 리뷰나 자도오하된 분석 도구를 사용하는 방법임
- 각 방법이 잘 찾아내지 못하는 오류를 찾아낼 수 있으니 다양한 방법을 동원해 테스트해야 오류를 최대한 줄일 수 있음
...
- Lock 인터페이스는 암묵적인 락과 달리 조건없는 락, 폴링 락, 타임아웃이 있는 락, 락 확보 대기 상태에 인터럽트를 걸 수 있는 방법 등이 포함되어 있고 락을 확보하고 해제하는 모든 작업이 명시적임
- ReentrantLock은 Lock인터페이스를 구현함. 따라서 ReentrantLock을 확보한다는 것은 synchronized블록에 진입하는 것과 동일한 효과를 갖고 있고 ReentrantLock을 해제한다는 것은 synchronized 블록에서 빠져나가는 것과 동일한 효과를 가짐
- ReentrantLock재진입이 가능함 그리고 Lock에 정의돼 있는 락 확보 방법을 모두 지원함
- synchronized 블록을 사용할 때보다 훨씬 능동적으로 대처 가능
- 반드시 finally 블록에서 락을 해제해야함
- ReentrantLock을 사용하면 해당하는 블록의 실행이 끝나고 통제권이 해당 블록을 떠나는 순간 락을 자동으로 해제하지 않기 때문에 굉장히 위험한 코드가 될 수 있음

### 폴링과 시간 제한이 있는 락 확보 방법

- 암묵적인락은 데드락이 걸리면 프로그램을 종료해야하지만 시간 제한, 또는 폴링 방법으로 락을 잡으면 데드락을 회피할 수 있는 방법으로 사용할 수 있음
...
- 양쪽 락을 모두 확보하도록 되어있는데 만약 동시에 확보가 불가능한 경우 잠시 대기했다가 재시도하도록 되어 있음
- 만약 지정된 시간동안 락을 확보하지 못했다면 적절한 에러를 낼 수 있음
...
- 일정한 시간안에 락을 확보하도록 하는 예제

### 인터럽트 걸 수 있는 락 확보 방법

- 인터럽트에 전혀 반응하지 않는 방법밖에 없다면 작업 도중 취소시킬 수 있어야만 하는 기능을 구현할 때 굉장히 복잡해짐
...
- Lock 인터페이스의 lockInterruptibly() 메서드를 활용하는 모습. 이제 cancellableSendOnSharedLine() 은 취소가 가능한 상태가 됨



### 블록을 벗어나는 구조의 락

- 암묵적인 락은 블록 내에서 반드시 끝나지만 복잡한 애플리케이션에서는 더 유연하게 동작해야함
- 핸드오버락, 락 커플링
...
- 자바버전이 올라가면서 암묵적인락과 ReentrantLock의 성능은 별 차이가 없음
- ReentrantLock이 더 좋은 성능이였음
- 그러나 오래된 자료이기때문에 오늘 실행해보면 또 다를 수 있음



## 공정성

- ReentrantLock는 불공정 락, 공정한방법 두가지를 제공함 기본값은 불공정
- 공정한 방법을 사용할 때는 요청한 순서를 지켜가면서 락을 확보함
- 불공정은 순서 뛰어넘기가 일어나기도 하는데 락을 확보하려고 대기하는 큐에 대기중인 스레드가 있다 하더라도 해제된 락이 잇으면 대기자 목록을 뛰어 넘어 락을 확보할 수 있음
- 그렇다고 ReentrantLock이 일부러 순서를 뛰어넘고 그러지는 않음
- 대부분의 경우 공정하게 순서를 관리해서 얻는 장점보다 불공정하게 처리해서 얻는 성능상의 이점이 큼
- 스레드간의 경쟁이 심하게 나타나는 상황에서 락을 공정하게 관리하는 것보다 불공정하게 관리하는 방법의 성능이 훨씬 빠른 이유는 대기 상태에 잇던 스레드가 다시 실행 상태로 돌아가고 또한 실제로 실행되기 까지는 상당한 시간이 걸리기 때문임
- 공정한 방법으로 락을 관리할 때는 락을 확보하고 사용하는 시간이 상대적으로 길거나 락 요청이 발생하는 시간 간격이 긴 경우에 유리함



### synchronized 또는 ReentrantLock선택

- 암묵적인 락은 여전히 명시적인 락에 비해서 상당한 장점을 갖고 있음
- 익숙하면서 간결하고 두개를 섞어 쓴다면 오히려 혼동되고 오류가 발생할 가능성도 더 높아짐
- ReentrantLock은 암묵적 락보다 더 위험할 수 있음
- 다음과같음 고급 동기화 기법을 사용해야 하는 경우에만 ReentrantLock을 사용할 것
  1. 락을 확보할 때 타임아웃을 지정해야 하는 경우
  2. 폴링 형태로 락을 확보하고자 하는 경우
  3. 락을 확보하느라 대기 상태에 들어가 있을 때 인터럽트를 걸 수 있어야 하는 경우
  4. 대기 상태 큐 처리 방법을 공정하게 해야 하는 경우
  5. 코드가 단일 블록의 형태를 넘어서는 경우
- 위 방법외에는 synchronized방법을 사용할 것



## 읽기-쓰기 락

- ReentrantLock은 표준적인 상호 배제 락을 구현하고 있음 즉 한 시점에 단 하나의스레드만이 락을 확보할 수 있음
- 상호 배제 규칙은 너무 엄격하고 보수적인 규칙임
- 대부분의 애플리케이션 작업은 읽기 작업이 많음. 이런 작업에 쓰기 적합한 락이 바로 ReadWriteLock임
- ReadWriteLock은 여러개의 읽기 작업은 동시에 처리할 수 있지만, 쓰기 작업은 한 번에 단 하나만 동작할 수 있음
- 일반적인 경우 월등한 성능을 보이지만 특정 상황을 놓고 보면 성능이 떨어질 수 있으니 직접 프로파일링을 해서 알아봐야함
- ReadWriteLock을 구현했을때 적용할 수 있는 특성
  - 락 해제 방법: 쓰기 작업에서 락을 해제했을때 대기하고있던 읽기 작업, 쓰기 작업중 누구에게 락을 먼저 넘겨줄 것인가의 문제
  - 읽기 순서 뛰어넘기: 읽기 작업에서 락을 사용하고 있고 대기 큐에 쓰기 작업이 대기중이라면 읽기 작업이 추가로 실행됐을 때 어떤 작업을 먼저 할것인지?
  - 재진입 특성: 읽기 작업과 쓰기 작업 모두 재진입이 가능한지?
  - 다운그레이드: 특정 스레드에서 쓰기 락을 확보하고 있을때 스기 락을 해제하지 않고도 읽기 락을 확보할 수 있는지? 만약 가능하다면 쓰기 작업을 하던 스레드가 읽기 락을 직접  확보하고 읽기 작업을 할 수 있음. 즉 읽기 락을 확보하려는 사이에 다른 쓰기 작업이 실행되지 못하게 할 수 있음
  - 업그레이드: 읽기 락을 확보하고 있는 상태에서 쓰기 락을 확보하고자 할 때 대기 큐에 들어 있는 다른 스레드보다 먼저 쓰기 락을 확보하게 할 것인지? 직접적인 업그레이드 연산을 제공하지 않는 한 자동으로 업그레이드가 일어나면 데드락의 위험이 높기 떄문에 ReadWriteLock을 구현하는 대부분의 경우에 업그레이드를 지원하지 않음





- ReentrantReadWriteLock을 사용하면 읽기 락과 쓰기 락 모두 재진입 가능한 락 기능을 제공함
- 공정성도 지정할 수 있음
- 읽기락을 확보하고 있는 상태에서 다른 스레드가 쓰기 락을 요청하는 경우 쓰기 락을 요청한 스레드가 쓰기 락을 확보하고 해제하기 전에는 다른 스레드에서 읽기 락을 가져가지 못함
- 불공정하게 설정된 락이라면 어느 스레드가 가져가는지 알 수 없음
- 락을 확보한 상태에서 읽기 락을 사용하는 다운그레이드는 허용되고 읽기 락을 확보한 상태에서 쓰기 락을 사용하는 업그레이드는 제한됨
- ReentrantReadWriteLock는 쓰기 락을 확보한 스레드만이 쓰기 락을 해제 가능
- 자바6 이상부터는 어느 스레드가 읽기 락을 확보했는지 추적하도록 되어 있음
...
- 실제 사용 예
- 하지만 Map 클래스가 필요했던 것이라면 oncurrentHashMap만 사용해도 충분히 괜찮은 성능을 낼 수 잇음
...
- synchronized에서 지원하지 않는 기능이 필요할때만 ReentrantLock을 사용할 것
- 읽기-쓰기 락을 사용하면 읽기 작업만 처리하는 다수의 스레드는 동기화된 값을 얼마든지 동시에 읽어갈 수 있음 따라서 읽기 작업이 대부분인 데이터 구조에 읽기 쓰기 락을 사용하면 확장성을 높여주는 훌륭한 도구가 됨
...
- 병렬 프로그램에서는 상태 기반의 조건은 다른 스레드를 통해서 언제든지 마음대로 변경될 수 있음
- 병렬 객체의 상태 종속적인 메서드는 선행 조건이 만족하지 않았을 때 오류가 발생하는 일보다는 선행 조건을 만족할때까지 대기하는 경우가 많음
- 원하는 상태에 다다들 때까지 반복문을 사용하는 대신 조건 큐를 사용하면 더 많은 이득을 볼 수 있음
- 선행 조건에 해당하는 클래스 내부의 상태 변수는 값을 확인하는 동안에도 적절한 락으로 반드시 동기화해야 올바른 값을 확인할 수 있음. 
...
- 위 BaseBoundedBuffer클래스는 아래에 올 모든 예제에 부모가됨



### 예제: 선행 조건 오류를 호출자에게 그대로 전달
...
- GrumpyBoundedBuffer을 사용한다고 했을때 선행조건이 맞지 않으면 SLEEP_GRANULARITY만큼 기다리는 코드를 클라이언트 측에서 작성해야함
- 대기시간만큼 대기하지 않고 take 메서드를 즉시 호출하는 스핀 대기 방법을 사용할 수 있는데 버퍼의 상태가 빨리 돌아오지 않는다면 상당한 양의 cpu 자원을 소모할 수 있음 하지만 대기시간을 너무 늘려버리면 과다 대기 문제가 생기기도함
- 따라서 호출자는 cpu를 덜 사용하되 응답성에서 손해를 보거나 응답성은 좋지만 cpu를 엄청나게 소모하는 두가지 방법 가운데 어떤 것을 사용할지 선택해야 함
- Thread.yield 메서드를 호출하면 시스템의 스케줄러에게 다른 스레드를 실행하려면 지금이 고내찮은 시점이다 라는 사실을 알리는 것과 같음 즉 다른 스레드가 뭔가 작업을 해주기를 기다리고 있는 상태라면 할당받은 스케줄 시간을 모두 사용하기 전에 다른 스레드를 먼저 실행시키는 방법도 나쁘지는 않음
...
- 이번에는 호출자가 신경쓰지 않고도 폴링 대기를 하는 기능을 내장시킨 SleepyBoundedBuffer 클래스
- 외부에서 간단하게 사용할 수 있다는 건 굉장히 중요함
- 잠시 대기하고 상태 조건을 확인하는 반복문을 계속해서 실행하다 조건이 적절해지면 반복문을 빠져나와 작업을 처리함
- 호출자에서 챙겨야할 일은 InterruptedException가 발생할 경우 처리하는 일임
- 이와 같이 폴링하고 대기하는 반복 작업을 통해 블로킹 연산을 구현하는 일은 상당히 고생스러운 일임 이런 일을 더 쉽게 구현하는 방법은 조건 큐를 사용하는 것



### 조건 큐 - 문제 해결사

- 조건 큐는 여러 스레드를 한 덩어리로 묶어 특정 조건이 만족할 때까지 한꺼번에 대기할 수 있는 방법을 제공하기 때문에 조건 큐라는 이름으로 불림
- 데이터 값으로 일반적인 객체를 담아두는 보통의 큐와 달리 조건 큐에는 특정 조건이 만족할 때까지 대기해야 하는 스레드가 값으로 들어감
- 자바에서 모든 객체는 스스로를 조건 큐로 사용할 수 있고 모든 객체가 갖고 있는 wait, notify, notifyAll 은 조건 큐의 암묵적인 api라고 봐도 좋음
- X라는 객체의 조건큐 api를 호출하고자 하면 반드시 객체 X의 암묵적인 락을 확보하고 있어야만 함 상태 기반의 조건이 만족하기를 기다리도록 구현된 부분이 객체 내부의 상태를 일관적으로 유지하도록 구현된 코드와 필연적으로 굉장히 밀접하게 관련돼 있기 때문임
- Object.wait 메서드는 "나는 대기 상태에 들어갈 예정인데 만약 뭔가 재미있는 일이 생기면 깨워주기 바랍니다" 라는 뜻이고 notify, notifyAll메서드는 "뭔가 재미있는 일이 발생했습니다" 라고 알려주는 셈
...
- 이전에 sleep으로 구현한 메서드보다 구현하기도 쉽고 훨씬 효율적이면서 응답성도 좋은편임
- cpu 사용의 효율성, 컨텍스트 스위치 관련 부하, 응답 속도 등의 측면에서 봤을 때 어느정도 최적화된것일뿐 sleep을 사용하는 이전 버전과는 작동하는 모습엔 변화가 없음



## 조건 큐 활용

- 조건큐를 사용하면 효율적이면서 응답속도도 빠른 상태 종속적인 클래스를 구현할 수 있지만 올바르지 않은 방법으로 사용할 가능성도 높음



### 조건 서술어

- 조건 큐를 올바로 사용하기 위한 가장 핵심적인 요소는 바로 해당 객체가 대기하게 될 조건 서술어를 명확하게 구분해내는 일
- 문서 어디에도 조건서술어에 대한 얘기가 없으나 predicate가 없으면 조건부 대기 기능이 동작할 수 없음
- 조건서술어는 애초에 특정 기능이 상태 종속적이 되도록 만드는 선행 조건을 의미함 크기가 제한된 버퍼를 예로 들면 take 메서드의 입장에서는 작업을 진행하기 전에 확인해야만 하는 "버퍼에 값이 있어야 한다" 는 것이 조건 서술어임
- 조건 큐와 연결된 조건 서술어는 항상 문서로 남겨야하며, 그 조건 서술어에 영향을 받는 메서드가 어느것인지도 명시해야함
- 조건 서술어는 상태 변수를 기반으로 하고 있고, 상태 변수는 락으로 동기화돼 있으니 조건 서술어를 만족하는지를 확인하려면 반드시 락을 확보해야만 함
- wait 메서드는 먼저 락을 해제하고 현재 스레드를 대기 상태에 두고 일정 시간 이후에 타임아웃이 발생하거나 스레드에 인터럽트가 걸리거나 notify 또는 notifyAll을 통해 알림을 받을때까지 대기함
- wait 메서드를 호출하는 모든 경우에는 항상 조건 서술어가 연결돼 있음. 특정 조건 서술어를 놓고 wait 메서드를 호출할 때, 호출자는 항상 해당하는 조건 큐에 대한 락을 이미 확보한 상태여야함. 또한 확보한 락은 조건 서술어를 확인하는 데 필요한 모든 상태변수를 동기화하고 있어야함



### 너무 일찍 깨어나기

- 하나의 암묵적인 조건 큐를 두 개 이상의 조건 서술어를 대상으로 사용할수도 있음 
- 위와 같은 경우는 wait이 리턴된다고 해서 wait에 해당하는 조건 서술어를 만족하지 않았을 수도 있음. 어디가에서 notifyAll을 호출했을 수 있기 때문.
- 하나의 암묵적인 조건 큐를 두 개 이상의 조건 서술어를 사용하면 여러개의 경우의 수가 더 생김 이런 방법은 매우 흔하게 사용됨
- 모든 원인 때문에 wait 메서드가 깨어나 리턴되고 나면 조건 서술어를 한번 더 확인해야함 따라서 매번 반복할 때마다 계속해서 조건 서술어를 확인해줘야함
- 조건부 wait 메서드를 사용할 때 주의사항
  - 항상 조건 서술어를 명시해야함
  - wait 메서드를 호출하기 전에 조건 서술어를 확인하고, wait에서 리턴된 이후에도 조건 서술어를 확인해야함
  - wait 메서드는 항상 반복문 내부에서 호출해야함
  - 조건 서술어를 확인하는 데 관련된 모든 상태 변수는 해당 조건 큐의 락에 의해 동기화 돼 있어야함
  - wait notify, notifyAll 메서드를 호출할 때는 조건 큐에 해당하는 락을 확보하고 있어야 함
  - 조건 서술어를 확인한 이후 실제로 작업을 실행해 작업이 끝날 때까지 락을 해제해서는 안됨



### 놓친 신호

- 특정 스레드가 이미 true인 조건을 놓고 조건 서술어를 제대로 확인하지 못해 대기 상태에 들어가는 상황을 놓친 상황이라고함
- 스레드 a가 조건 큐에 신호를 보내주고 신호가 지나간 이후에 스레드 b가 동일한 조건 큐에서 대기한다면 스레드 b는 대기상태에서 깨어나지 못함
- wait 메서드를 호출하기 전에 반드시 조건 서술어를 확인할것



### 알림

- 위 BoundedBuffer의 take의 wait을 깨울 수 있는 부분은 put 메서드임 대기(take) <-> 알림(put)
- take에도 notifyAll이 있는 이유는 공간이 모자라 값을 추가하지 못하고 대기하는 스레드를 깨우는 일
- 특정 조건을 놓고 wait 메서드를 호출해 대기 상태에 들어간다면, 해당 조건을 만족하게 된 이후에 반드시 알림 메서드를 사용해 대기 상태에 빠져나오도록 해야함
- notify는 조건 큐에서 대기 상태에 있는 스레드 하나. notifyAll은 조건 큐에서 대기하는 모든 스레드를 풀어줌
- 여러 개의 스레드가 하나의 조건 큐를 놓고 대기 상태에 들어갈 수 있는데 대기 상태에 들어간 조건이 서로 다를 수 있기 때문에 notifyAll 대신 notify를 쓰는것은 위험성이 높음. 잘못하면 위에 설명한 놓친 상황이 연속적으로 발생할 수 있기 때문임
- notifyAll 대신 notify 메서드를 사용하려면 다음과 같은 조건에 해당하는 경우에만 사용하는 것이 좋음
  - 단일 조건에 따른 대기상태에서 깨우는 경우: 해당하는 조건 큐에 단 하나의 조건만 사용하고 있는 경우이고, 따라서 각 스레드는 wait 메서드에서 리턴될 때 동일한 방법으로 실행됨
  - 한번에 하나씩 처리하는 경우: 조건 변수에 대한 알림 메서드를 호출하고 나면 하나의 스레드만 실행시킬 수 있는 경우
- 대부분의 클래스는 위 두가지 조건을 모두 만족하지는 못하기 때문에 notifyAll을 사용하는 것이 좋음
- 하지만 notifyAll은 성능적으로도 생각해볼만한게 10개의 스레드가 notifyAll로 깨어난다면 10개 모두 하나의 락을 잡으려고 경쟁하고 락을 잡은 스레드를 제외한 9개의 스레드는 다시 대기상태에 들어감. 컨텍스트 스위칭이 빈번하게 일어나고 락 경쟁이 벌어짐
- 안전성과 성능, 트레이드 오프임
- 조건부 알림도 있는데 꽤나 복잡하고 섬세하게 사용하야함
...
- 조건부 대기 기능을 활용하면 여러 번 닫을 수 있는 ThreadGate와 같은 클래스를 어렵지 않게 구현할 수 있음
...
- 조건부 알림 기능이나 단일 알림 기능을 사용하고 나면 해당 클래스의 하위 클래스를 구현할 때 상당히 복잡해지는 문제가 생길 수 있음
- 상태 기반으로 동작하는 클래스는 하위 클래스에게 대기와 알림 구조를 완전하게 공개하고 그 구조를 문서로 남기거나 하위클래스에서 대기와 알림 구조에 전혀 접근할 수 없도록 깔끔하게 제거해야함
- 클래스를 final로 만드는 방법도 있음
...
- 상태를 기반으로 하는 모든 연산과 상태에 의존성을 갖고 있는 또 다른 상태를 변경하는 연산을 수행하는 경우에는 항상 진입 규칙과 완료 규칙을 정의하고 문서화해야함



## 명시적인 조건 객체

- 암묵적인 락을 일반화한 형태가 Lock 클래스인 것처럼 암묵적인 조건 큐를 일반화한 형태는 Condition 클래스임
- 암묵적인 조건 큐의 단점은 모든 암묵적인 락 하나는 조건 큐를 단 하나만 가질 수 있음
- 암묵적인 락이나 조건 큐 대신 Lock 클래스와 Condition 클래스를 활용하면 여러가지의 조건을 사용하는 병렬 처리 객체를 구현하거나 조건 큐를 노출시키는 것에 대한 공부를 할 때 훨씬 유연하게 대처할 수 있음
- Condition 클래스를 활용하면 하나의 락에 여러 조건으로 대기하게 할 수 있고 또한 인터럽트에 반응하거나 븐응하지 않는 대기상태, 데드라인을 정해둔 대기 상태, 공정하거나 공정하지 않은 큐 처리 방법 등 다양한 기능을 제공함
- Condition은 암묵적인 조건 큐와 달리 Lock하나를 대상으로 필요한 만큼 몇개라도 만들 수 있음
- Condition객체는 자신을 생성해준 Lock 객체의 공정성을 그대로 물려받음. 공정할경우 Condition.await에서 리턴할때 FIFO를 따름
- Condition 객체를 사용할때 주의할 점은 Object.wait을 호출하지 않도록하는게 중요함. 기능과 이름이 비슷한데 혼용해서 사용하면 동기화 기능에 큰 문제가 생길 수 있음
...
- 내부적으로 조건 큐를 사용하는 모습은 읽기도 쉽고 구조를 분석하기도 쉬움
- Condition 객체를 사용해서 단일 알림조건을 만들 수 있기 때문에 signalAll대신 효율적인 signal 메서드를 사용해 동일한 기능을 처리할 수 있어서 성능상으로도 이점을 줌
- 암묵적인 락이나 조건 큐와 같이 Lock클래스와 Condition객체를 사용하는 경우에도 락과 조건과 조건 변수 간의 관계가 동일하게 유지돼야함. 조건에 관련된 모든 변수는 Lock의 보호 아래 동기화돼 있어야 하고, 조건을 확인하거나 await 또는 signal 메서드를 호출하는 시점에는 반드시 Lock을 확보한 상태여야함
- 공정한 큐 관리 방법이나 하나의 락에서 여러 개의 조건 큐를 사용할 필요가 있는 경우라면 Condition 객체를 사용하고 그럴 필요가 없다면 암묵적인 조건 큐를 사용하는 편이 더 나음
...
- ReentrantLock과 Semaphore의 인터페이스는 비슷한 부분이 많음 실제로 양쪽 클래스 모두 다른 여러 동기화 클래스와 같이 AbstractQueuedSynchronizer를 상속받아 구현돼 있음
- AQS는 락이나 기타 동기화 클래스를 만들수 있는 프레임웍 역할을 하며 AQS를 기반으로 하면 엄청나게 다양한 종류의 동기화 클래스를 간단하면서 효율적으로 구현할 수 있음
- AQS 기반으로 만들어진 동기화 클래스는 대기 상태에 들어갈 수 있는 지점이 단 한군데이기 때문에 컨텍스트 스위칭 부하를 줄일 수 있고 결과적으로 전체적인 성능을 높일 수 있음



## AbstractQueuedSynchronizer

- 개발자가 AQS를 사용할 일은 거의 없음 그래도 알면 좋음
- AQS 기반의 동기화 클래스가 담당하는 작업 가운데 가장 기본이 되는 연산은 바로 확보acquire와 해제release임
- 확보 연산은 상태기반으로 동작하고 항상 대기 상태에 들어갈 가능성이 있음
- 락이나 세마포어 등의 입장에서 확보라는 연산은 락이나 퍼밋을 확보한다는 것으로 그 의미가 굉장히 명확함
- 이 연산을 사용하는 호출자는 항상 원하는 상태에 다다를 때까지 대기할 수 있다는 가능성을 염두에 두어야함
- 해제 연산은 대기 상태에 들어가지 않고 대신 확보 연산에서 대기중인 스레드를 풀어주는 역할을 함
- 특정 클래스가 상태 기반으로 동작하라면 반드시 상태 변수를 하나 이상 갖고 있어야함
- AQS는 ReentrantLock 처럼 배타적으로 동작할 수도 있고 Semaphore나 CountDownLatch에서와 같이 배타적이지 않을 수도 있음
- AQS에서는 조건 큐 기능을 지원하는 락을 간단하게 구현할 수 있도록 동기화 클래스와 연동된 조건 변수를 생성하는 방법을 제공함
...
- 확보는 await, 해제는 signal
- 누군가 signal을 호출해 해제 연산을 실행하면 그 동안 await에서 대기하던 스레드가 모두 해제되고 signal 호출 이후에 await를 호출하는 스레드는 대기 상태에 들어가지 않고 바로 실행됨
- 0이면 닫힌거고 1이면 열린 상태
- AQS의 핵심 기능을 위임하는 형식으로 사용하는게 더 좋은ㅁ 상속을 통한 구현 방법은 그다지 권장하지 않음



## java.util.concurrent 패키지의 동기화 클래스에서 AQS 활용 모습

- 래스와 같이 대기상태에 들어갈 수 있는 클래스는 AQS를 기반으로 되어 있음 ReentrantLock, Semaphore, ReentrantReadWriteLock, CountDownLatch, SynchronousQueue, FutureTask 등



### ReentrantLock

- 배타적인 확보 연산만 제공 따라서 tryAcquire, tryRelease, isHeldExclusively와 같은 메서드만 구현함
- ReentrantLock은 동기화 상태 값을 확보된 락의 개수를 확인하는데 사용하고 owner라는 변수를 통해 락을 가져간 스레드가 어느 스레드인지도 관리함
- tryRelease에서는 unlock을 호출하기 전에 owner 변수에 들어있는 내용을 들여다보고 해당 락을 확보하고 있는 스레드가 현재 스레드인지를 확인함.
- tryAcquire 메서드에서는 락을 확보하려는 시도가 재진입 시도인지 아니면 최초 락을 확보하려는 것인지 구분하기 위한 용도로 aowner 변수의 내용을 사용함
- tryAcquire는 compareAndSetState 메서드를 사용해 상태 값을 단일 연산으로 업데이트함
- 락의 상태를 확인했을 때 이미 확보된 상태라고 판단되면 락을 확보하고 있는 스레드가 현재 스레드인지를 확인하고 만약 그렇다면 락 확보 개수를 증가시킴
...
- tryAcquireShared 메서드는 먼저 현재 남아 있는 퍼밋의 개수를 알아내고, 남아 있는 퍼밋의 개수가 모자란다면 확보에 실패했다는 결과를 리턴함 반대로 충분한 개수의 퍼밋이 남아있었다면 compareAndSetState메서드를 사용해 단일 연산으로 퍼밋의 개수를 필요한 만큼 줄임 퍼밋의 개수를 줄이는 작업이 성공하면 확보 연산이 성공했다는 결과를 리턴함
- 메서드 내부의 while 반복문은 충분한 개수의 퍼밋이 없거나 tryAcquireShared 메서드가 확보 연산의 결과로 퍼밋 개수를 단일 연산으로 변경할 수 있을 때까지 반복함
- compareAndSetState 메서드를 호출했을 때 다른 스레드와 경쟁하는 상태였다면 값을 변경하지 못하고 실패할 수도 있으며 실패했다면 계속해서 재시도하게 되고 허용할만한 횟수 이내에서 재시도를 하다 보면 위의 두가지 조건 가운데 하나라도 만족하게됨
- 비슷하게 tryReleaseShared메서드는 퍼밋의 개수를 증가시키며 따라서 현재 대기 상태에 들어가 있는 스레드를 풀어줄 가능성도 있고 성공할 때까지 상태 값 변경 연산을 재시도함 tryReleaseShared메서드 리턴 결과를 보면 해제 연산에 따라 다른 스레드가 대기 상태에서 풀려났을 가능성 여부를 알 수 있음
- CountDownLatceh 클래스도 동기화상태 값을 현재 개수로 사용하는 Semaphore와 비슷한  형태로 AQS를 활용함
- countDwon 메서드는 release 메서드를 호출하고 release 메서드에서는 개수 값을 줄이고 개수가 0에 이르렀다면 대기중이던 스레드를 대기 상태에서 풀어줌 wait 메서드는 acquire 메서드를 호출하며 클래스 내부의 개수가 0이라면 즉시 리턴되고 0보다 큰 값이라면 대기 상태에 들어감



### FutureTask

- Future.get 메서드를 보면 래치 클래스와 굉장히 비슷한 기능을 갖고 있음
- 특정 이벤트가 발생하면 해당 스레드가 계속 진행할 수 있고 아니면 원하는 이벤트가 발생할 때까지 스레드가 대기상태에 들어감
- FutureTask는 실행중이거나 완료됐거나 취소되는 드으이 상황을 관리하는데 AQS 내부 동기화 상태를 활용함
- 작업이 끝나면서 만들어낸 결과 값이나 작업에서 오류가 발생했을 때 해당하는 예외 객체를 담아둘 수 있는 추가적인 상태 변수도 갖고 있음 
- 실제 작업을 처리하고 있는 스레드에 대한 참조도 갖고 있고 그래야 인터럽트 요청이 들어왔을때 해당 스레드에 인터럽트를 걸 수 있음

### ReentrantReadWriteLock

- AQS를 기반으로 구현된 ReentrantReadWriteLock은 사실 AQS하위 클래스 하나로 읽기 작업과 스기 작업을 모두 담당함
- 16비트는 쓰기락, 16비트는 읽기락 총 32비트 상태변수를 사용함
- 내부적으로 보면 AQS를 상속받은 클래스는 대기 중인 스레드의 큐를 관리하고
- 스레드가 독점적인 연산을 요청했는지 아니면 독점적이지 않은 연산을 요청했는지도 관리함.
- ReentrantReadWriteLock은 락에 여유가 생겼을 때 대기 큐의 맨 앞에 들어 있는 스레드가 쓰기 락을 요청한 상태였다면 해당 스레드가 락을 독점적으로 가져가고 만약 맨 앞에 있는 스레드가 읽기 락을 요청한 상태였다면 쓰기 락을 요청한 다음 스레드가 나타나기 전까지 읽기 락을 요청하는 모든 스레드가 독점적이지 않은 락을 가져감



## 요약

- 상태를 기반으로 동작하는 클래스, 즉 메서드 가운데 하나라도 상태 값아ㅔ 따라 대기 상태에 들어갈 가능성이 있는 클래스를 작성해야 할 때 가장 좋은 방법은 기존에 만들어져 있는 Semaphore, BlockingQueue, CountDownLatch등을 활용해 구현하는 방법
- 만약 적절한 기능을 찾을 수 없다면 암묵적인 조건 큐나 명시적인 Condition 클래스 또는 AQS클래스 등을 활용해 직접 원하는 기능의 동기화 클래스를 작성할 수도 있음
- 상태 의존성을 관리하는 작업은 상태의 일관성을 유ㅜ지하는 방법과 맞물려 있기 때문에 암묵적인 조건 큐 역시 암묵적인 락과 굉장히 멀접하게 관려돼 있음. 이와 비슷하게 명시적인 조건 큐인 Condition 클래스도 명시적인 Lock 클래스와 밀접하게 관련되어 있고 락 하나에서 다수의 대기 큐를 활용하거나 대기 상태에서 인터럽트에 어떻게 반응하는지를 지정하는 기능, 스레드 대기 큐의 관리 방법에 대한 공정성 여부를 지정하는 기능, 대기 상태에서 머무르는 시간을 제한할 수 이쓴ㄴ 기능 등과 같이 암묵적인 버전으 조건 큐나 락 보다 훨씬 다양한 기능을 제공함

# #15 단일 연산 변수와 넌블로킹 동기화

- java.util.concurrent 패키지에 들어있는 다수의 클래스는 단순하게 synchronized 구문으로 동기화를 맞춰 사용하는 것에 비하면 속도도 빠르고 확장성도 좋음
- 위 패키지에 있는 클래스들의 성능이 좋아진 원인은 넌블로킹 동기화 기법임
- 넌블로킹 알고리즘은 락을 기반으로 하는 방법보다 설계와 구현 모두 훨씬 복잡하며, 대신 확장성과 활동성을 엄청나게 높여줌
- 여러 스레드가 동일한 자료를 놓고 경쟁하는 과정에서 대기 상태에 들어가는 일이 없기 떄문에 스케줄링 부하를 대폭 줄여줌
- 데드락, 기타 활동성 문제가 발생할 위험도 없음
- 락을 기반으로 하는 알고리즘은 특정 스레드가 락을 확보한 상태에서 잠자기, 반복문을 실행하면 그 시간동안 각자의 작업 가운데 락이 필요한 부분을 전혀 실행할 수 없지만 넌블로킹 알고리즘을 사용하는 경우에는 개별 스레드에서 발생하는 오류에 의해 영향을 받는 일이 없음
- AtomicInteger나 AtomicReference등의 단일 연산 변수를 사용해 넌블로킹 알고리즘을 효율적으로 구현할 수 있게 됨
- 단일 연산 변수는 본격적인 넌 블로킹 알고리즘을 구현하는 일이 아니라 해도 더 나은 volatile변수의 역할만으로 사용할 수 있음
- 단일 연산 변수는 volatile변수와 동일한 메모리 유형을 갖고 있으며 이에 덧붙여 단일 연산으로 값을 변경할 수 있는 기능을 갖고 있음 따라서 이런 특성을 사용해 숫자 카운터, 일련번호 생성기, 통계 수치 추출기 등으로 활용하면 락 기반의 구조에 비해 높은 확장성을 얻을 수 있음

## 락의 단점

- 공유된 상태에 접근하려는 스레드에 일관적인 락 구조를 적용해 동기화하면 특정 변수를 보호하고 있는 락을 확보한 스레드가 해당 변수에 대한 독점적인 접근 권한을 갖게 되고, 변수의 값을 변경했다고 하면 다음 스레드가 락을 확보했을 때 모든 변경 사항을 완벽하게 볼 수 있음
- 락을 기반으로 세밀한 작업을 주로 하도록 구현돼 있는 클래스는 락에 대한 경쟁이 심해질수록 실제로 피룡한 작업을 처리하는 시간대비 동기화 작업에 필요한 시간의 비율이 상당한 수치로 높아질 가능성이 있음
- volatile변수는락과비교해 봤을 때 컨텍스트 스위칭이나 스레드 스케줄링과 아무런 연관이 없기 때문에 락보다 훨씬 가벼운 동기화 방법이라고 볼 수 있음 하지만 단일연산만 가능하기 떄문에 하나의 변수가 다른 변수와 관련된 상태로 사용해야 하거나 하나의 변수라도 해당 변수의 새로운 값이 이전 값과 관련이 있다면 volatile 변수를 사용할 수 없음 따라서 제한되는 부분이 많음
- 복합연산을 하나의 단일 연산으로 쓰려면 락을 사용해야함
- 스레드가 락을 확보하기 위해 해당 스레드는 아무런 작업을 할 수 없다는 것도 단점
- 우선순위가 낮은 스레드가 락을 오랫동안 보유하고 있으면 우선순위가 높은 스레드는 마치 우선순위가 낮은 스레드처럼 동작하게됨 우선순위 역전현상
- 최악의 상황에서는 락을 확보하고 있던 스레드가 영원히 멈출 수 있음 이때 대기하던 모든 스레드 역시 영원히 대기하고 동작을 멈춤

## 병렬 연산을 위한 하드웨어적인 지원

- 배타적인 락 방법은 보수적인 동기화 방법.
- 세밀하고 단순한 작업을 처리하는 경우에는 일반적으로 훨씬 효율적으로 동작할 수 있는 낙관적인 방법이 있는데, 일단 값을 변경하고 다른 스레드의 간섭 없이 값이 제대로 변경되는 방법 충돌 검출 방법을 사용해 값을 변경하는 동안 다른 스레드에서 간섭이 있었는지를 확인할 수 있고 만약 간섭이 있었다면 해당 연산이 실패하게되고 이후에 재시도하거나 아예 재시도조차 하지 않기도함
- 멀티프로세서 연산을 염두에 두고 만들어진 프로세서는 공유된 변수를 놓고 동시에 여러 작업을 해야 하는 상황을 간단하게 관리할 수 있도록 특별한 명령어를 제공함. compare and swap or load-linked/store/conditional 등

### 비교 후 치환

- CAS연산에는 3개의 인자를 넘겨주는데 작업 대상 메모리의 위치인 V 예쌍하는 기존 값인 A, 새로 설정할 값인 B
- CAS연산은 V위치에 있는 값이 A와 같은 경우에 B로 변경하는 단일연산
- 만약 이전 값이 A와 달랐다면 아무런 동작도 하지 않음. 그리고 값을 B로 변경했건 못했건 간에 어떤 경우라도 현재 V의 값을 리턴함
- CAS는 낙관적
- 동시에 여러 스레드가 한 변수에 CAS를 사용하면 한 스레드빼고 모두 실패함. 하지만 대기하지 않고 다시 시도할 수 있다고 통보를 받는 셈
- CAS연산이 실패한다면 이후 동작을 결정할 수 있음
- CAS연산의 유연성 때문에 락을 사용하면서 밟생할 수밖에 없었던 여러 가지 활동성 문제를 미연에 방지할 수 있음
...
- 대기상태에 들어갈 일이 없는 CasCounter 클래스는 객체를 계속해서 업데이트 하고 있다면 여러 차례 재시도 해야 할 수 있음
- CAS기반의 클래스가 락 기반의 클래스보다 성능이 훨씬 좋고 경쟁이 없는 경우에도 락 기반의 방법보다 나은 경우가 있음
- 락은 jvm에서 비용이 높음 <= 요즘도 그러나?
- CAS 연산을 프로그램에서 직접 사용하면 jvm, 운영체제, 스케줄링 관련 작업을 따로 할 필요가 없음
- CAS 연산의 가장 큰 단점은 호출하는 프로그램에서 직접 스레드 경쟁 조건에 처리를 해야한다는점. 반면 락을 사용하면 락을 사용할 수 있을 떄까지 대기 상태에 들어가도록 하면서 스레드 경쟁 문제를 알아서 처리해준다는 차이점이 있음
- CAS연산의 성능은 프로세서 마다 크게 차이가남 <= 지금쯤 많이 좋아졌을듯



### JVM에서의 CAS연산 지원

- JVM은 CAS연산을 호출받았을 때 해당하는 하드웨어에 적당한 가장 효과적인 방법으로 처리하도록 돼 있음



## 단일 연산 변수 클래스

- 단일 연산 변수 atomic variable는 락보다 훨씬 가벼우면서 세밀한 구조를 갖고 있고 멀티프로세서 시스템에서 고성능의 병렬 프로그램을 작성하고자 할 때 핵심적인 역할을 함
- 단일 연산 변수를 사용하면 스레드가 경쟁하는 범위를 하나의 변수로 좁혀주는 효과가 있고 이 정도의 범위는 프로그램에서 할 수 있는 가장 세밀한 범위
- 락 대신 단일 연산 변수를 기반의 알고리즘으로 구현된 프로그램은 내부의 스레드가 지연되는 현상이 거의 없고 스레드 간의 경쟁이 발생한다 해도 훨씬 쉽게 경쟁 상황을 해쳐나갈 수 있음
- 단일 연산 변수 클래스는 volatile 변수에서 읽고 변경하고 쓰는 것과 같은 조건부 단일 연산을 지원하도록 일반화한 구조임.
- 12개의 단일 연산 변수 클래스가 제공되고 대략 일반 변수, 필드 업데이터, 배열, 조합 변수의 4개의 그룹으로 나눌 수 있음
- 가장 많이 사용하는 형태는 AtomicInteger, AtomicLong, AtomicBoolean, AtomicReference클래스임
- 제공되지 않는 형태의 변수 short이나 byte의 경우 강제로 int로 변환해서 읽거나 쓰면 되고 정수가 아닌 숫자를 사용하는 경우에는 Float.floatToIntBits 메서드, Double.doubleToLongBits 메서드를 사용하면 됨
- 단일 연산 배열 변수 클래스는 배열의 각 항목을 단일 연산으로 업데이트할ㄹ 수 있도록 구성돼 있는 배열 클래스임 배열에 대한 참조가 volatile임
- 단일연산 클래스는 Number를 상속받음
- 단일 연산 클래스는 hashCode, equals 메서드를 재정의하고 있지 않고 가변이기때문에 해시 값으로는 적합하지 않음



### '더 나은 volatile' 변수로의 단일 연산 클래스

- 범위라는 조건은 항상 두 변수의 값을 동시에 사용해야 하고 필요한 조건을 만족하면서 그와 동시에 양쪽 범위 값을 동시에 업데이트 할 수 없기 떄문에 volatile 참조를 사용하거나 AtomicInteger를 사용한다 해도 확인하고 동작하는 연산을 안전하게 수행할 수 없음
...
- ReentrantRock과 AtomicInteger의 성능 비교를 위한 난수 발생기예쩨
...
- 실험을 해보면 경쟁이 많은 상황에서는 단일 연산 변수보다 락이 더 빠르게 처리되는 모습을 볼 수 있지만 훨씬 실제적인 경쟁 상황에서는 단일 연산 변수가 락보다 성능이 더 좋음 이런 현상은 락의 특성 때문에 나타나는데 락을 두고 경쟁이 발생하면 대기 상태에 들어가는 스레드가 나타나고 일부 스레드가 대기 상태에 들어가면서 전체적인 cpu 사용률과 공윧된 메모리 버스의 동기화 트래픽이 줄어드는 효과에 의해 처리 속도가 높아짐
- 단일 연산 변수를 사용하면 경쟁 조건에 대한 처리 작업의 책임이 경쟁하는 스레드에게 넘어감
- 경쟁이 적거나 보통의 경쟁 수준에서는 단일 연산 변수를 사용해야 확장성을높일 수 있고 경쟁이 수준이 아주 높은 상황에서는 락을 사용하는 쪽이 경쟁에 더 잘 대응하는 모습을 보임
- 제일빠른건 ThreadLocal 상태변수를 공유하지 않는게 가장 빠름
- 스레드 간의 경쟁을 최대한 적절하게 처리하면 확장성을 어느 정도 향상시킬 수 있음. 하지만 최종적으로 확장성을 가장 높일 수 있는 방법은 스레드 간의 경쟁이 발생하지 않도록 미연에 방지하는 방법임



## 넌블로킹 알고리즘

- 락 기반으로 동작하는 알고리즘은 항상 다양한 종류의 가용성문제이 직면할 위험이 있음
- 특정 스레드에서 작업이 실패하거나 또는 대기 상태에 들어가는 경우에, 다른 어떤 스레드라도 그로 인해 실패하거나 대기 상태에 들어가지 않는 알고리즘을 넌블로킹 알고리즘이라고 함
- 각 작업 단계마다 일부 스레드는 항상 작업을 진행할 수 있는 경우 락 프리 알고리즘이라고 함
- 스레드 간의 작업 조율을 위해 CAS 연산을 독점적으로 사용하는 알고리즘을 올바로 구현한 경우에는 대기 상태에 들어가지 ㅇ낳는 특성과 락 프리 특성을 함께 가지게 됨
- 여러 스레드가 경쟁하지 않는 상황이라면 CAS 연산은 항쌍 성공하고, 여러 스레드가 경쟁을 한다고 해도 최소한 하나의 스레드는 반드시 성공하기 때문에 성공한 스레드는 작업을 진행할 수 있음.
...
- 넌블로킹 알고리즘을 구성할 떄 가장 핵심이 되는 부분은 바로 데이터의 일관성을 유지하면서 단일 연산 변경 작업의 범위를 단 하나의 변수로 제한하는 부분임
...
- 단일 연산 참조를 사용해 스택을 어떻게 구현하는지를 보여주는 좋은 예
- CAS 연산이 성공하거나 실행하는 어떤 경우라 해도 위 예제는 항상 안정적인 상태를 유지함



### 넌블로킹 연결 리스트

- 넌블로킹 알고리즘을 작성할 때의 핵심은 바로 단일 연산의 범위를 단 하나의 변수로 제한하는 부분
- 연결 큐 같은 경우는 head, tail 두개의 참조가 있어야하는데 단일연산 변수로는 처리할 수 없음 따라서 연결 큐를 대기 상태에 들어가지 않도록 구현할 수 있는 알고리즘은 두가지 경우를 모두 처리할 수 있어야함
- 이런 기능을 구현하려면 여러가지 전략을 동원해야함 데이터 구조가 여러 단계의 변경 작업을 거치는 과정을 포함해 언제라도 일관적인 상태를 유지하도록 해야함
...
- 마이클 스콧의 넌블로킹 연결 큐 알고리즘.
- ConcurrentLinkedQueue 클래스에서도 사용하고 있는 알고리즘임
- 어느 스레드건 간에 tail.next 값을 확인해보면 해당 큐가 어떤 상태에놓여 있는지를 확인할 수 있고 어느 스레드가 항목을 새로 추가하는 작업을 하던 간에 항상 원하는 작업을 제대로 끝마칠 수 있음



### 단일 연산 필드 업데이터

- 업데이터 클래스에 지정한 클래스의 지정 변수가 업데이터 클래스를 통하지 않고 직접 변경되는 경우가 있다면 연산의 단일성을 보장할 수 없음
- 따라서 필드 업데이터 클래스로 지정한 변수에 대해 연산의 단일성을 보장하려면 모든 스레드에서 해당 변수의 값을 변경할 때 항상 compareAndSet메서드나 기타 산술 연산 메서드를 사용해야만 함



### ABA 문제

- ABA 문제는 노드를 재사용하는 알고리즘에서 비교 후 치환 연산을 고지식하게 사용하다보면 발생할 수 있는 이상 현상을 말함
- 참조 값 하나만 변경하는 것이 아니라 참조와 버전 번호의 두가지 값을 한꺼번에 변경하면 쉽게 해결 가능함



## 요약

- 대기 상태에 들어가지 않는 넌블로킹 알고리즘은 락 대신 비교 후 치환과 같은 저수준의 명령을 활용해 스레드 안전성을 유지하는 알고리즘임.
- 이런 저수준의 기능은 특별하게 만들어진 단일 연산 클래스를 통해 사용할 수 있고 단일 연산 클래스는 더나은 변수로써 정수형 변수나 객체에 대한 참조 등을 대상으로 단일 연산 기능을 제공하기도함
- 넌블로킹 알고리즘은 설계하고 구현하기는 훨씬 어렵지만 특정 조건에서는 훨씬 나은 확장성을 제공하기도 하고, 가용성 문제를 발생시키지 않는다는 점이 있음
- JVM이나 플랫폼 자체의 라이브러리에서 대기 상태에 들어가지 않는 알고리즘을 적절히 활용하는 범위가 넓어지면서 JVM의 버전이 올라갈 때마다 병렬 프로그램의 성능이 계속해서 나아지고 있음



# #16 자바 메모리 모델

- 안전한 공개, 동기화 정책 등 상위 개념이 제공하는 안전성은 모두 자바 메모리 모델을 기반으로 하고 있고 자바 메모리 모델의 내부 구조가 어떻게 동작하는지를 이해하고 있다면 상위 개념을 훨씬 효율적으로 쉽게 사용할 수 있음

## 자바 메모리 모델은 무엇이며, 왜 사용해야 하는가?

- 멀티스레드로 실해오디는 환경에서는 성능을 크게 제한하지 않는 한 순차성이 주는 안전성과 높은 성능은 찾아보기 어려움
- 병렬 프로그램이라 하더라도 대부분의 시간은 스레드 내부에서 각자의 작업을 처리하기 떄문에 스레드 간의 작업 조율 기능에 자원을 많이 낭비하는 일은 별 이득도 없고 프로그램의 성능만 떨어뜨리는 결과를 낳기 십상
- 스레드 간의 작업을 조율하는데 꼭 필요한 데이터만을 공유해 사용하는 것이 올바른 방법
- JMM은 변수에 저장된 값이 어느 시점부터 다른 스레드의 가시권에 들어가는지에 대해 JVM이 해야만 하는 최소한의 보장만 할 뿐
- JMM은 예측성에 대한 필요와 함께 높은 성능의 JVM을 다양한 종류의 프로세서 구조에서 동작하도록 해야 한다는 실제적인 요구 사항을 쉽게 구현할 수 있어야 한다는 점의 균형을 맞출 목적으로 설계됨

### 플랫폼 메모리 모델

- 메모리를 공유하는 멀티프로세서 시스템은 보통 각자의 프로세서 안에 캐시 메모리를 갖고 있고, 캐시 메모리의 내용은 주기적으로 메인 메모리와 동기화됨
- 하드웨어 프로세서 아키텍처는 저마다 다른 캐시 일관성을 지원함. 동일한 시점에 각 프로세서가 다른 값을 읽어가는 경우를 허용하기도 함
- 자바개발자가 서로 다른 하드웨어가 갖고 있는 각자의 메모리 모델을 직접 신경쓰지 않도록 자바는 스스로의 메모리 모델인 JMM을 구성하고 있고 JMM과 그 기반이 되는 하드웨어 메모리 모델의 차이점은 메모리 배리어를 적절히 활용하는 방법 등으로 JVM에서 담당해 처리함
- 프로그램이 실행되는 내용을 예상하기에 가장 간편한 방법은 하드웨어 프로세서에 상관 없이 프로그램 내부에 작성된 코드가 실행되는 단 한가지의 방법이 존재하고 그 과정에서 변수에 마지막으로 설정한 값을 어떤 프로세서건 간에 정확하게 읽어낼 수 있다고 가정하는 방법. 비현실적이긴 하지만 이처럼 꿈같이 간편한 상태를 순차적 일관성이라함
- 소프트웨어 개발자는 무의식적으로 순차적 일관성이 존재한다고 생각하지만 어떤 프로세서도 순차적 일관성을 지원하지 않고 JMM역시 지원하지 않음 
- 자바로 프로그램을 작성하는 과정에서 메모리 배리어를 어디에 어떻게 배치해야 하는지를 고민할 필요는 없음 단지 적절한 동기화 방법을 사용해 어느시점에서 공유된 정보를 사용하는지만 알려주면 됨

### 재배치

- JMM은 서로 다른 스레드가 각자의 상황에 맞는 순서로 명령어를 실행할 수 있도록 허용하고 있기 때문에 동기화가 돼 있지 않은 부분을 놓고 실행 순서를 예측하는 일이 훨씬 더 복잡해졌음 이런 특정 작업이 지연되거나 다른 순서로 실행되는것처럼 보이는 문제는 재배치 라는 용어로 통일해서 표현함
...
- x와 y의 값을 예측하기가 매우어려움 어느 스레드간 재배치가 일어날 수 있음
- 동기화가 제대로 되지 않은 상태에서 재배치될 가능성을 예측하는 일은 너무나 어렵고 반대로 동기화 방법을 적절하게 사용해 재배치될 가능성을 없애는 일이 더 쉬움
- 동기화가 잘 된 상태에서는 컴파일러, 런타임, 하드웨어 모드 JMM이 보장하는 가시성 수준을 위반하는 쪽으로 메모리 관련 작업을 재배치하지 못하게 됨



### 자바 메모리 모델을 간략하게 설명한다면

- JMM에서는 프로그램 내부의 모든 작업을 대상으로 미리 발생이라는 부분 재배치 연산을 정의하고 있음 작업 A가 실행된 결과를 작업 B에서 볼 수 있다는 점을 보장하기 위해 작업 A와 B사이에는 미리 발생 관계가 갖춰져야함
- 두 개 작업 간에 미리 발생 관계가 갖춰져 있지 않다면 JVM은 원하는 대로 해당 작업을 재배치 할 수 있게 됨
- 하나의 변수를 두 개 이상의 스레드에서 읽어가려고 하면서 최소한 하나 이상의 스레드에서 쓰기 작업을 하지만 쓰기 작업과 읽기 작업 간에 미리 발생 관계가 갖춰져 있지 않은 경우에 데이터 경쟁 현상이 발생함
- 이런 데이터 경쟁 현상이 발생하지 않는 프로그램을 올바르게 동기화된 프로그램이라 하고 이런 프로그램들은 순차적 일관성을 갖고 있고 프로그램 내부의 모든 작업이 고정된 전역 순서에 따라 실행된다는 것을 의미함
- 미리 발생 현상에 대한 규칙
  - 프로그램 순서 규칙: 특정 스레드를 놓고 봤을 때 프로그램된 순서에 앞서있는 작업은 동일 스레드에서 뒤에 실행되도록 프로그램된 작업보다 미리 발생함
  - 모니터 잠금 규칙: 특정 모니터 잠금 작업이 뒤이어 오는 모든 모니터 잠금 작업보다 미리 발생함
  - volatile 변수 규칙: volatile 변수에 대한 쓰기 작업은 이후에 따라오는 해당 변수에 대한 모든 읽기 작업보다 미리 발생함
  - 스레드 시작 규칙: 특정 스레드에 대한 Thread.start 작업은 시작된 스레드가 갖고 있는 모든 작업보다 미리 발생함
  - 스레드 완료 규칙: 스레드 내부의 모든 작업은 다른 스레드에서 해당 스레드가 완료됐다는 점을 파악하는 시점보다 미리 발생함. 특정 스레드가 완료됐는지를 판단하는 것은 Thread.join 메서드가 리턴되거나 Thread.isAlive 메서드가 false를 리턴하는지 확인하는 방법을 말함
  - 인터럽트 규칙: 다른 스레드를 대상으로 interrupt 메서드를 호출하는 작업은 인터럽트 당한 스레드에서 인터럽트를 당했다는 사실을 파악하는 일보다 미리 발생함. 인터럽트를 당했다는 사실을 파악하려면 InterruptedException을 받거나 isInterrupted 메서드 또는 inteerupted 메서드를 호출하는 방법을 사용할 수 있음
  - 완료 메서드 규칙: 특정 객체애 대한 생성 메서드가 완료되는 시점은 완료 메서드가 시작하는 시점보다 미리 발생함
  - 전이성: A가 B보다 미리 발생하고 B가 C 보다 미리 발생한다면 A는 C보다 미리 발생함
- 두개의 스레드 A와 스레드 B가 하나의 락을 공유한다면 스레드 A와 스레드 B의 모든 작업은 프로그램 내부의 규칙에 따라 순서가 정해져 있음
...
- 락으로 보호돼 있지 않은 변수에 접근해 사용하는 순서를 정의할 때 모니터 락이나 volatile 변수 규칙과 같은 여러 가지 순서 규칙에 미리 발생 규칙을 함께 적용해 순서를 정의하는 방법을 말함
- 이런 기법은 명령이 나열된 순서에 굉장히 민감하며 오류가 발생하기 쉽기 때문에 되도록 안쓰는게 좋음
- FutureTask 클래스에 AbstractQueuedSynchronizer의 메서드가 피기백 방법으로 구현되어 있음
...
- 객체가 안전하지 않게 공개되는 이유는 공유 객체를 공개하는 작업과 다른 스레드에서 공개된 객체를 사용하는 작업 간의 미리 발생 관계를 제대로 적용하지 못했기 때문임

### 안전하지 못한 공개

- 미리 발생 관계를 제대로 고려하지 못한 상태에서 재배치 작업이 일어날 수 있는 가능성을 놓고 보면 적절한 동기화 구조를 갖추지 못하고 공개된 객체를 두고 다른 스레드에서 부분 구성된 객체를 볼 수밖에 없는 원인이 쉽게 설명됨
...
- 늦은 초기화로인한 안전하지 않은 공개
- 불변 객체가 아닌 이상, 특정 객체를 공개하는 일이 그 객체를 사용하려는 작업보다 미리 발생하도록 구성돼 있지 않다면 다른 스레드에서 생성하 ㄴ객체를 사용하는 작업은 안전하지 않음

### 안전한 공개

- 객체를 공개하는 작업이 다른 스레드에서 해당 객체에 대한 참조를 가져다 사용하려는 작업보다 미리 발생하도록 만들어져 있기 때문에 공개된 객체가 다른 스레드에게 올바른 상태로 보인다는 것을 뜻함



### 안전한 초기화를 위한 구문

- 늦은 초기화 기법을 잘못 사용하면 안됨
...
- 위와 같이 synchronized를 사용해서 해결 가능
- static으로 선언된 변수에 초기화문장을 함께 기술하는 방법을 사용하면 스레드 안전성을 추가로 보장받을 수 있음
- static으로 선언된 초기화 문장은 jvm에서 해당 클래스를 읽어들이고 실제 해당 클래스를 사용하기 전에 실행됨
- 이런 초기화 과정에서 jvm이 락을 확보하며 각 스레드에서 해당 클래스가 읽혀져 있는 지를 확인하기 위해 락을 다시 확보하게 돼 있음 따라서 jvm이 락을 확보한 상태에서 메모리에 쓰여진 내용은 모든 스레드가 볼 수 있음
- 위의 내용은 static 이 불변이거나 가변이더라도 항상 동기화를 맞춰줘야 할때만 다른스레드에서 올바르게 볼 수 있음
...
- 이런식으로 한다면 synchronized키워드마저 제거할 수 있음
...
- 위와 같이 사용한다면 ResourceHolder를 사용하기 전까지는 해당 클래스를 초기화하지 않음

### 더블 체크 락

- 초창기JVM에서 사용하던 기법, DCL
- DCL은 먼저 동기화 구문이 없는 상태로 초기화 작업이 필요한지를 확인하고 resource 변수의 값이 null이 아니라면 resource 변수에 참조된 객체를 사용함. 만약 초기화 작업이 필요하다면 동기화 구문을 사용해 락을 걸고 Resource 객체가 초기화됐는지 다시한번 확인하는데 이렇게하면 객체 초기화작업은 한번에 하나의 스레드만 가능하긴 함 문제는 이미 만들어진 Resource 인스턴스에 대한 참조를 가져오는 부분은 동기화되어있지 않다는 점
...
- 이런 코드는 쓰면 안됨
- 차라리 쓸꺼면 늦은 초기화 홀더 클래스를 쓸 것



## 초기화 안전성

- 초기화 안전성을 보장한다는 의미는 올바르게 생성된 불변 객체를 어떤 방법으로건, 심지어는 데이터 경쟁이 발생하는 방법으로 공개하더라도 여러 스레드에서 별다른 동기화 구문 없이 안전하게 사용할 수 있다는 의미
- 초기화 안전성을 확보하지 못하는 경우 악의적인 프로그램이 보안 검증 과정을 통과하도록 하는 보안상의 허점이 되기도함
- 초기화 안전성이 확보돼 있다면 완전하게 구성된 객체를 대상으로 해당 객체가 어떻게 공개됐던 간에 생성 메서드가 지정하는 모든 final 변수의 값을 어떤 스레드건 간에 올바르게 읽어갈 수 있다는 점을 보장함. 또한 완전하게 구성된 객체 내부에 final로 선언된 객체를 거쳐 사용할 수 있느 모든 변수 역시 다른 스레드에서 안전하게 볼 수 있다는 점도 보장됨
...
- 별다른 동기화를 하지 않고 스레드 안전하지 않은 HashSet을 사용한다 해도 이를 대상으로 안전하지 않은 늦은 초기화 작업을 진행하거나 동기화 구문 없이 SafeStates에 대한 참조를 public static 으로 선언된 변수에 선언하는 것으로도 안전하게 공개할 수 있음
- 만약 states 변수가 final이 아니거나 변경 가능하다면 동기화구문을 추가하지않는 이상 안전하지 않게됨
- 초기화 안전성은 생성 메서드가 오나료되는 시점에 final로 선언된 변수와 해당 변수를 거쳐 접근할 수 있는 값에 대해서만 가시성을 보장함. final로 선언되지 않은 변수나 생성 메서드가 종료된 이후에 변경되는 값에 대해서는 별도의 동기화 구문을 적용해야 가시성을 확보할 수 있음

## 요약

- 자바 메모리 모델은 특정 스레드에서 메모리를 대상으로 취하는 작업이 다른 스레드에게 어떻게 보이는지의 여부를 명시하고 있음
- 가시성을 보장해주는 연산은 미리 발생이라는 규칙을 통해 부분적으로 실행 순서가 정렬된 상태를 유지하고 미리 발생 규칙은 개별적인 메모리 작업이나 동기화 작업의 수준에서 정의 하는 수준임
- 충분히 동기화되지 않은 상태에서는 공유된 데이터를 여러 스레드에서 사용할 때는 굉장히 이상한 현상이 발생할 수 있음
