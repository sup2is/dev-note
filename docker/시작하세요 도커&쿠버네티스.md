...
- 생성된 컨테이너의 이름은 (프로젝트이름\_서비스이름\_서비스 내에서 컨테이너 번호) 로 생성됨
...
- ports: docker run 명령어의 -p와 같음 단일 호스트 환경에서 호스트의 특정 포트를 연결하면 scale 할 수 없음\
...
- spec.replicas: 동일한 포드를 몇 개 유지시킬 것인지 설정함 
...
- 서비스종류에 따라 포드에 접근할 수 있는 방법이 달라지기 때문에 서비스 타입 세개는 반드시 익힐 것
  ...
  - LoadBalancer 타입: 클라우드 플랫폼에서 제공하는 로드 밸런서를 동적으로 프로비저닝해 포드에 연결함. NodePort 타입과 마찬가지로 외부에서 포드에 접근할 수 있는 서비스타입임 일반적으로 AWS, GCP등과 같은 클라우드 플랫폼 환경에서만 사용 가능



### ClusterIP 타입의 서비스 - 쿠버 내부에서만 포드에 접근하기
...
- spec.selector : selector 항목은 이 서비스에서 어떠한 라벨을 가지는 포드에 접근할 수 있게 만들 것인지 결정함 위 예시에서는 app:webserver라는 라벨을 가지는 포드들의 집합에 접근할 수 있는 서비스를 생성함 이 라벨과 디플로이먼트의 포드들의 라벨과 매칭되어 이 서비스의 의해 접근 가능한 대상으로 추가됨
- spec.ports.port: 생성된 서비스는 쿠버 내부에서만 사용할 수 있는 고유한 IP를 할당받음 port 항목에는 서비스의 IP에 접근할 때 사용할 포트를 설정함
- spec.ports.targetPort : selector 항목에서 정의한 라벨에 의해 접근 대상이 된 포드들이 내부적으로 사용하고 있는 포트를 입력함 포드 템플릿에 정의된 containerPort와 같은 값으로 설정해야함
- spec.type: 이 서비스가 어떤 타입인지를 나타냄 서비스의 세종류로 가능
...
curl 10.97.85.55:8080 --silent | grep Hello
==
curl hostname-svc-clusterip:8080 --silent | grep Hello
```

- kbuectl get services로 서비스 확인 가능
- curl을 여러번 사용해보면 알겠지만 포드끼리 알아서 로드밸런싱됨
- 아이피 말고 같은 네트워크 상에 있기때문에 도메인명으로 호출한느게 일반적

1. 특정 라벨을 가지는 포드를 서비스와 연결하기 위해 서비스의 yaml 파일에 selector 항목을 정의함
2. 포드에 접근할 때 사용하는 포트를 yaml 파일의 targetPort 항목에 정의함 == containerPort
3. 서비스를 생성할 때 yaml 파일의 port 항목에 8080을 명시해 서비스의 cluster ip와 8080 포트로 접근할 수 있게 설정함
4. kubectl apply -f 명령어로 ClusterIP 타입의 서비스가 생성되면 서비스는 쿠버 클러스터 내부에서만 사용할 수 있는 고유한 내부 IP를 할당받음
5. 쿠버 클러스터에서 서비스의 내부 ip 또는 서비스의 이름을 포드에 접근할 수 있음
...
- NodePort 타입의 서비스는 클러스터 외부에서도 접근 가능함
- 모든 노드의 특정 포트를 개방해 서비스에 접근하는 방식
- 스웜모드에서 컨테이너를 외부로 노출하는 방식과 비슷함
...
- ClusterIp타입과 다른점이 없음
- kubectl get services 로 매핑된 포트 확인 가능
- 포트는30000~ 32768 포트중 랜덤으로할당되고 지정 가능함
- ClusterIP 타입의 서비스의 상위타입이 NodePort타입임 따라서 NodePort타입을 사용해도 ClusterIP 타입의 서비스 기능을 사용할 수 있음
- 80과 443으로 설정하기에 적절하지도 않고 실제로 잘 사용하는 편은 아님



### 클라우드 플랫폼의 로드밸런서와 연동하기 - LoadBalancer 타입의 서비스

- LoadBalancer타입의 서비스는 서비스 생성과 동시에 로드 밸런서를 새롭게 생성해 포드와 연결함
- GCP, AWS 등과 같은 클라우드 플랫폼 환경에서만 사용 가능 
...
1. LoadBalancer 타입의 서비스가 생성됨과 동시에 모든 워커 노드는 포드에 접근할 수 있는 랜덤한 포트를 개방함
2. 클라우드 플랫폼에서 생성된 로드 밸런서로 요청이 들어오면 이 요청은 쿠버네티스의 워커 노드 중 하나로 전달되며 이때 사용되는 포트는 1번에서 개방된 포트인 32620임
3. 워커 노드로 전달된 요청은 포드중 하나로 전달되어 처리됨

- LoadBalancer 타입을 사용해도 NodePort의 간접적인 기능또한 사용할 수 있음



**온프레미스 환경에서 LoadBalancer 타입의 서비스 사용하기**

- 온프레미스 서버에도 LoadBalancer를 사용하려면 MetalLB라는 이름의 오픈소스를 사용해야함
- 쿠버 공식 프로젝트가 아니여서 유지보수가 지속적이지 않을 수 있음

### 트래픽의 분배를 결정하는 서비스 속성: externalTrafficPolicy

- LoadBalancer 타입의 서비스를 사용하면 외부로부터 들어온 요청은 각 노드 중 하나로 보내지며 그 노드에서 다시 포드 중 하나로 전달됨
- externalTrafficPolicy를 Local로 설정하면 포드가 생성된 노드에서만 포드로 접근할 수 있고 로컬 노드에 위치한 포드 중 하나로 요청이 전달됨 
- Local과 Cluster 모드 둘 다 장단점이 있으니 고려해볼만 함



### 요청을 외부로 리다이렉트하는 서비스 : ExternalName

- 쿠버를 외부 시스템과 연동해야 할 때는 ExternalName타입의 서비스를 사용할 수도 있음
...
- 도커나 도커스웜모드에는 논리적으로 컨테이너를 구분하는 방법이 없었지만 쿠버에서는 네임스페이스라는 오브젝트로 논리적으로 구분 가능함
- 간단하게 네임스페이스는 포드, 레플리카셋, 디플로이먼트, 서비스 등과 같은 쿠버네티스 리소스들이 묶여있는 하나의 가상 공간 또는 그룹이라고 이해하면 됨
...
- 기본적으로 default라는 namespace를 가짐
- kube-system 네임스페이스는 쿠버 클러스터 구성에 필수적인 컴포넌트들과 설정값 등이 존재하는 네임스페이스임
...
- 실제 사용은 metadata 안에 namespace 옵션으로 사용 가능

**네임스페이스의 서비스에 접근하기**

- 같은 네임스페이스 내부에서 서비스는 접근 가능이지만 다른 네임스페이스로 접근할 경우 {서비스이름}.{네임스페이스 이름}.svc 처럼 서비스 이름 뒤에 네임스페이스 이름을 붙이면 다른 네임스페이스의 서비스에도 접근 가능
- kubectel delete -f {yaml 파일} 또는 kubectl delete namespace 명령어로 간단히 삭제 가능하지만 내부 리소스들은 전부 제거되니 주의



**네임스페이스에 종속되는 쿠버네티스 오브젝트와 독립적인 오브젝트**

- 모든 리소스가 네임스페이스에 의해 구분되는 것은 아님 포드, 서비스, 레플리카셋, 디플로이먼트는 구분 가능
- 예를 들어 A라는 네임스페이스에 포드를 만들면 A 네임스페이스에서만 보이고 B에서는 안보임 이런 경우를 쿠버에서는 오브젝트가 네임스페이스에 속한다 라고 표현함
- kubectl api-resources --namespaced=true
- node같은건 네임스페이스에 의해 논리적으로 구분되지 않음



## 컨피그맵, 시크릿: 설정값을 포드에 전달

- 쿠버는 yaml파일과 설정값을 분리할 수 있는 컨피그맵과 시크릿이라는 오브젝트를 제공함 컨피그맵에는 설정값을, 시크릿에는 노출되어서는 안되는 비밀 값을 저장할 수 있음

### 컨피그맵

**컨피그맵 사용 방법 익히기**

- 컨피그맵은 일반적인 설정값을 담아 저장할 수 있는 쿠버 오브젝트이며 네임스페이스에 속하기 때문에 네임스페이스별로 컨피그 맵이 존재함
- kubectl create configmap 명령어를 사용해서 쉽게 컨피그맵을 생성할 수 있음

```
kubectl create configmap log-level-configmap --from-literal LOG_LEVEL=DEBUG 
kubectl create configmap start-k8s --from-literal k8s=kube --from-literal container=docker

kubectl get cm 으로 확인
...
- 포드를 적용할땐 몇가지 설정값이 있음
  - **컨피그맵의 값을 컨테이너의 환경 변수로 사용:** 컨피그맵의 값을 포드의 컨테이너 환경 변수로 가져옴 echo ${key}로 사용 가능
  - **컨피그맵의 값을 포드 내부의 파일로 마운트해 사용:** 컨피그맵의 값을 포드 컨테이너 내부의 특정 파일로 마운트함 어떤 파일로 떨굴때 좋은 방법
...
- spec.volumes: yaml 파일에서 사용할 볼륨의 목록을 정의함 위 예시에서는 start-k8s 라는 이름의 컨피그맵을 통해 configmap-volume 볼륨을 정의했음 여기서 정의한 볼륨은 spec.containersm 항목에서 참조해 사용
- spec.containers.volumeMounts: volumes 항목에서 정의된 볼륨을 컨테이너 내부의 어떤 디렉터리에 마운트할 것인지 명시함 
