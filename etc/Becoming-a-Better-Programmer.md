...
- 완료상태란 어떤것이고 완료 상태에 얼마나 가까운지를 현실적으로 파악하는것이 중요함
- 관리자가 언제끝나냐고 물어봤을때 명확하게 완료시점을 알고 의사소통할줄알아야함

## 거꾸로 개발하기 : 분해

- 완료 상태를 정의하기 위한 첫 번째 단계는 작업 내용을 정확히 파악하는 것
- 엄청나게 큰 업무를 받았다면 일을 시작하기 전에 더 작고 이해할만한 부분으로 일을 나누도록 할 것
- 분해작업은 어렵지만 중요함 반드시 초기에 이루어져야함
- 언제나 작업의 최소 단위를 파악할 것

## '완료' 정의하기

- 어떤 일을 하고 있든 간에 그만두어야 할 시점을 파악할 것
- 완료 상태를 정의해야함
- 그만둘 때를 결정하지 않는다면 필요 이상으로 작업하게 됨
- 무엇이 성공인지 파악하기 전까지는 코딩 작업을 일절 시작하지 말 것 아직 모르겠다면 완료 상태가 무엇인지 정의하는 것부터 시작
- 어디로 향하는지 알아야만 집중할 수 있고 명확한 방향을 향해 작업할 수 있음
- 완료 상태의 기준
  - 명확성
    - 명확하고 구체적이어야함.
    - 구현해야할 모든 기능을 목록화 해야함.
  - 가시성
    - 모든 주요 관련자들이 성공 기준을 확인하도록 할것
    - 모든 사람이 이 기준을 알고 그에 동의하도록 할 것
  - 실현 가능성
    - 완료 기준을 주의 깊게 정의할 것
    - 실현 불가능한 완료 상태를 정의하는 것은 무의미함
- 코드 작성이 완료되었고 작동함을 증명하기 위해 코드로 작성된 테스트를 사용할 것
- 완료를 설명할 때 고려해야할 사항
  - 어디로 코드를 전달하는가? 버전관리
  - 어디에 코드를 배포하는가? 라이브 서버에 배포가 완료?
  - 완료했음을 어떻게 알리는가?

## 그냥 실행하라

- 완료 상태를 정의해야만 집중적으로 작업을 수행할 수 있음. 완료 지점까지만 작업하고 필요한 것보다 더 많은 일을 하지 말것
- 코드가 충분하면 그만 멈출것 완벽하지 않아도됨
- 코드가 정확하지 않다면 리팩토링해야하지만 아직 수정하지 말 것
...
- 그 어떤 개발자도 혼자가 아님
- 너무 좁은 시야로 너무 가깝게 문제의 일부를 바라보면 문제 전체를 보지 못하거나 효율적으로 문제에 대처하지 못하게 되므로 주의해야함
- 자신을 감시할것. 최소 시간과 기한을 정하고 작업 도중에 경과를 확인할 것
- 다른 프로그래머에게 의무감을 갖고 그들과 주기적으로 작업 경과를 확인할 것
- 자신감으로 충만한 나머지 도움 요청을 놓치는 일이 없도록 할 것

## 산 아래 서서

- 문제에 직면했을 때 이를 해결하기 위한 한 가지 이상의 접근법을 고려해야함
- 그런 다음 작업에 착수해야함
...
- 훌륭한 프로그래머가 되고 싶다면 훌륭한 프로그래머들 주변에 의도적으로 머물것
- 훌륭한 프로그래머들에게 얻을 수 있는 것
  - 확산되는 열정
  - 영감을 주는 동기 부여
  - 전염되는 책임감
  - 좋은 프로그래밍 습관
...
- 전문가들은 모든 시간을 일하는데 쏟지 않음. 진짜 전문가는 일을 쉬운 방향으로 이끌어나감

## 과거로부터의 통찰

- 좋은 코더와 함꼐 일하면 좋은 코드로 일을 마무리할 가능성이 매우 높음
...
- 작업의 품질을 보증하기 위해 다른 프로그래머들에 대한 의무감을 가지면 코드 품질을 환상적으로 높일 수 있음

## Code++

- 훌륭한 코드를 작성하고 있음을 확신하려면 모든 단계에 걸쳐 확인해줄 사람이 필요함
- 가장 중요한 것은 의무감의 필요성을 인지하는 것

## 작동하게 만들기

- 다른 사람이 코드를 읽고 품평하리라는 것을 알고 나면 좋은 코드를 짜고 싶은 마음이 커짐
- 모든 코드를 두 명 이상이 검수한 후에 소스 관리 도구에 체크인하도록 할 것

## 기준 정하기

## 다음 단계

- 의무감은 좋은 것이라는 의견에 동의하고 의무감을 갖는 데 전념할 것
- 의무감의 대상이 될 누군가를 찾을 것
- 모든 코드를 두 명 이상이 확인하도록 할 것
- 어떻게 의무감을 구축할지에 대해 합의할 것. ex 회의, 주간리뷰, 설계 회의, 페어프로그래밍, 코드리뷰
- 어느 수준 이상의 작업에 전념하고 그에 대한 논쟁에 대비할것. 방어적으로 대응하지 말라



## 마치며

- 프로그래머 상호 간의 의무감을 위해서는 일정 수준 이상의 용기가 필요함.
- 비판을 받아들여야하고 비판을 적절히 수행하기 위해 전략적이어야함
...
- 소프트웨어 그 자체 즉 코드를 작성하는 행위야말로 의사소통의 한 형태임



### 기계와의 대화

- 적절한 결과를 원한다면 적절한 명령을 남겨야함
- 코드는 컴퓨터와의 의사소통임. 명확하고 애매모호함이 없어야만 의도대로 명령이 수행될 것임

### 동물과의 대화

- 코드는 다른 사람들과의 의사소통임. 명백하고 애매모호함이 없어야만 다른 사람들이 코드를 유지보수할 수 있음
- 역량있는 프로그래머는 자신의 의도를 명확하게 전달하는 코드를 작성하고자함
- 코드는 알기 쉬워야하고. 알고리즘을 노출하고 로직을 가리지 않고 다른 이들이 쉽게 수정할 수 있어야함
- 더 많은 주석을 단다고 반드시 코드가 더 나아지지는 않음. 의사소통에 충실한 코드는 추가적인 주석이 필요 없음
- 같은 코드 영역을 대상으로 작업하는 팀은 반드시 같은 언어로 작성해야함
- 코드는 작성이 아닌 읽기에 최적화되어야함
- 의도를 명확히 드러내는 레이아웃 규칙을 따라야함
...
- 여러 형태의 의사소통 수단을 터득하고 각각의 대화에 적합한 수단을 사용할 것. ex 이메일, 전화 등등

### 언어에 주목하라

- 적절한 사람들과 적절한 단어를 사용하도록 신경쓸 것. ex 도메인 용어

### 바디 랭귀지

### 병렬적 의사소통

- 효과적인 커뮤니케이션을 위해서는 집중해야함

## 팀원과의 대화

- 팀의 의사소통 구조에 따라 코드의 형태가 결정됨
- 좋은 의사소통에 좋은 코드가 만들어짐. 의사소통의 형태가 코드의 형태를 만들어낼 수 있음
- 다른사람들에게 솔직하고 건전한 태도로 이야기해야만 효과적인 팀워크를 이룰 수 있음
- 오히려 짧은 일일 회의가 더 좋음
...
- 개발자는 고객이 원하는 바를 이해해야함.
- 일단 물어보고 지속적인 대화를 통해 여전히 원하고 있는지 그리고 기대를 충족시키고 있는지를 확인해야함
- 고객의 언어로 소통할 것
...
- 효과적인 의사소통의 특징
  ...
  - 적절한 수준에서
  - 적절한 매체를 통한 소통
- 개발자는 이들 요소를 염두에 둔 채 의사소통을 연습해야함.
...
- 개발 방법론, 최신 경향, 선언문, 유행에 대해 알아둘것

## 하지만 유일한 진리인가

- 납득할만한 개발 선언문들을 지지할것. 하지만 맹목적으로 따르거나 독단적으로 다루지 말 것 
- 그 어떤 선언문이든 원칙에 대한 폭 넓은 선언일 뿐 유일한 진리의 길은 절대 아님
- 그들로부터 배우고 그들이 지지하는 개밸에 대한 견해를 이해할것
- 자신에게 적합한 것을 사용하고 최신 정보를 유지할 것
- 새로운 관습과 유행하는 교리에 대해 배울 것
- 그들에게 좋은 점은 감사하며 받아들이되 무턱대고 따르지는 말것.
...
- 저자의 선언문
  ...
  - 그 무엇도 고정된 것은 없다
  - 꾸준히 배우라
  - 주체가 자신이든 팀이든 코드이든 간에 나아질 방향을 꾸준히 모색하라
...
- 코더에 적대적으로 반응하던 제럴드는 결국 혼자남았고 프로젝트 일정을 끝내지 못해 감방에 간 슬픈 이야기 ...

## 코딩은 사람의 문제다

- 보통 소프트웨어 개발과 관련해 까도로운 부분은 기술적인 측면에 있지 않음. 결국 사람의 문제
- 코드에 신경쓰지 않는 코드에게 둘러싸여 있다면, 자신만이라도 건전한 태도를 유지하라.
- 코드에 신경쓰지 않는 사람에게 방법을 알려줄것 특히 비난처럼 느껴지지 않는 방법을 택해야함
...
- 좋은 프로그래머와 나쁜 프로그래머를 구분하는 요소는 바로 태도임
- 기술적으로 아무리 경쟁력 있는 프로그래머라고 해도 건전한 태도를 바탕으로 하지 않는다면 좋은 결과를 기대하기 어려움
- 당신의 태도가 당신이 프로그래머로서 얼마나 나아갈 수 있는지를 결정함. 더 나은 프로그래머가 되고 싶다면 더 나은 태도를 목표로 할 것

## 시작하라. 코드를 작성하라

- 좋은 코드에 주의를 기울이고 더 나은 방법을 찾으라. 언제나 배우라. 설계 방법을 배우고, 코드를 작성하는 방법을 배우고, 같이 일하는 방법을 배우라. 언제나 시험에 들게 하고 힘을 북돋아주는 훌륭한 개발자들과 함께 일하려 노력하라. 근면하고 양심적이며 전문적이 되도록하라
- 프로그래밍을 즐겨라. 무엇보다 더 나아짐을 즐겨라!!! 
