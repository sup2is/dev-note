...
![9-1](./images/headfirst-design-pattern-revised-edition/9-1.png)
...
![9-2](./images/headfirst-design-pattern-revised-edition/9-2.png)
...
- 반복자 패턴을 사용하면 내부 구조를 드러내지 않으면서도 클라이언트가 컬렉션 안에 들어있는 모든 원소에 접근하도록 할 수 있다.
- 컴포지트 패턴은 개별 객체와 복합 객체를 모두 담아둘 수 있는 구조를 제공한다.
- 컴포지트 패턴을 사용하면 클라이언트가 개별객체와 복합 객체를 똑같은 방법으로 다룰 수 있다.
...
![11-1](./images/headfirst-design-pattern-revised-edition/11-1.png)
...
- 원격 프록시
  - 클라이언트와 원격 객체 사이의 데이터 전달을 관리해준다.
- 가상 프록시
  - 인스턴스를 만드는 데 많은 비용이 드는 객체로의 접근을 제어한다.
- 보호 프록시
  - 호출하는 쪽의 권한에 따라서 객체에 있는 메서드로의 접근을 제어한다.
