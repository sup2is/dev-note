...
- 이 책의 초판은 1999년이다. 훌륭한 내용임에는 틀림이 없지만 그 동안 프로그래밍 관련 용어들도 많이 바뀐 것 같아서 엄청나게 잘 읽히는 느낌은 아니었다. (영문 표현은 그대로일 수 있지만 번역이? 예를 들면 '마법숫자', '변경불가 객체')   
- 이어서 긴 시간동안 java version이 많이 올라갔다. 책에서는 제네릭, foreach 등등의 문법이 존재하지 않는다.
- 책 내용중에서 '이 변수를 사용하는 부분을 전부 변경해야하는 어려움이 있다.' 와 같은 내용이 있는데 IDE가 너무 발전해서 사실 크게 공감이 안간다... 쩝
- 하지만 분명 좋은 내용들이다. 1판이 있기 때문에 1판을 읽었지만 2판을 읽는게 좋을 것 같다. (2판은 javascript로 되어 있지만 이해하는데 큰 어려움이 없다고 한다.)
