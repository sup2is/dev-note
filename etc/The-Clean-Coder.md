...
- 무술에서의 카타: 격투 중인 한 사람을 가상해 일련의 잘 짜인 동작을 모은 것
- 프로그래밍에서의 카타: 어떤 문제 풀이를 가상해 만든 키 누름과 마우스 조작을 정교하게 짜 모은 것
- 카타 != 문제풀이
...
- 자신의 영향력을 넓히지 못하면 자신의 경력과 사고 방식이 해로울정도로 좁아진다.

`오픈소스`

- 변화의 물결을 따라잡는 한 가지 방법은 소스 프로젝트에 기여해 공익에 봉사하는 것이다.
- 오픈소스는 수없이 많고 다른 사람들이 관심을 갖는 일에 적극적으로 뛰어드는 행동은 자신의 기술 목록을 다양화하는 최고의 방법이다.
- 자바 프로그래머라면 Rails 프로젝트에 기여해라.
