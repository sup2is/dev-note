...
클라우드에서 실행되는 마이크로서비스의 구성관리는 항상 자동화되어야 한다.
...
3. **중앙 집중화** 클라우드 기반의 어플레키엿ㄴ에는 말 그대로 수백개의 서비스가 존재할 수 있으므로 구성 정보를 보관하는 저장소 개수를 최소로 줄이는 것이 매우 중요하다. 어플리케이션의 구성 정보를 가능한 소수 저장소에 집중화 한다.
