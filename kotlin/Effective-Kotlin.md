...
- ## 아이템 46: 함수 타입 파라미터를 갖는 함수에 inline 한정자를 붙여라
  ...
  - inline이 아닌 함수는 내부에서 리턴을 사용할 수 없다.
  ...
  - 이는 함수 리터럴이 컴파일될 때, 함수가 객체로 래핑되어서 발생하는 문제다. 함수가 다른 클래스에 위치하므로  return을 사용해서 main으로 돌아올 수가 없다.
  ...
  - inline 한정자는 굉장히 유용하지만 모든 곳에 사용할 수는 없다.

    - 인라인함수는 재귀적으로 동작할 수 없다. 재귀적으로 사용하면 무한하게 대체되는 문제가 발생한다.
    ...
    - 인라인 함수는 더 많은 가시성 제한을 가진 요소를 사용할 수 없다. private과 internal 가시성을 가진 함수와 프로퍼티를 사용할 수 없다.
  ...
  - 함수를 인라인으로 만들고 싶지만 어떤 이유로 일부 함수 타입 파라미터는 inline으로 받고 싶지 않은 경우엔 다음과 같은 한정자를 사용한다.

    - crossinline: 아규먼트로 인라인 함수를 받지만 비지역적 리턴을 하는 함수를 받을 수 없게 만든다. 인라인으로 만들지 않은 람다 표현식과 조합해서 사용할 때 문제가 발생하는 경우 활용한다.
  ...
  - 인라인 함수가 사용되는 주요 사례
    ...
    - 타입 아규먼트를  reified 타입으로 전달받는 경우
    - 함수 타입 파라미터를 갖는 톱레벨 함수를 정의해야 하는 경우 특히 컬렉션 처리와 같은 헬퍼 함수, 스코프 함수, 톱레벨 유틸리티 함수의 경우
  - 인라인 함수가 다른 인라인 함수를 호출하는 경우 코드가 기하급수적으로 많아질 수 있기 때문에 주의하자.
...
- 매니지드 언어라도 '더 이상 사용하지 않는 객체의 레퍼런스를 유지하면 안된다' 라는 규칙 정도는 지켜주는게 좋다.

- 큰 객체를 정적으로 갖고 있으면 가비지 컬렉터가 이를 제거할 수 없다. 큰 객체는 정적으로 유지하지 않는것이 좋다.

- 사용하지 않는 오브젝트에 null을 설정하는 것은 그렇게 어려운 일이 아니므로 무조건 하는 것이 좋다.

- 가독성이 좋은 코드

  - 가독성이 좋다면 메모리와 성능적으로도 좋다.
  - 가독성이 좋지 않다면 메모리와 리소스의 낭비를 숨기고 있을 가능성이 높다.

- SoftReference

  - 소프트 레퍼런스를 활용하면 메모리가 필요한 경우에는 가비지 컬렉터가 이를 알아서 해제한다.
  - WeakReference는 가비지 컬렉터가 값을 정리하는 것을 막지 않는다. 다른 레퍼런스가 이를 사용하지 않으면 곧바로 제거된다.

- 메모리 누수는 예측하기 어렵다. 프로파일링 도구를 활용하자.

- 메모리와 관련된 문제를 피하는 가장 좋은 방법

  - 변수를 지역스코프에 정의한다.
  - 톱레벨 프로퍼티 또는 companion 객체에 큰 데이터를 저장하지 않는다.
...
- immutable컬렉션보다 mutable 컬렉션이 좋은 점은 성능적인 측면에서 더 빠르다는 것이다.
- immutable은 요소를 추가하면 새로운 컬렉션을 만들면서 요소를 추가한다.
- 복제는 상당한 비용이므로 이러한 복제처리를 하지 않는 mutable은 성능적 관점에서는 좋다. 하지만 안전하지 않다.
- 지역변수로 사용할 때는 mutable 컬렉션을 사용하는 것이 더 합리적이라고 할 수 있다.
