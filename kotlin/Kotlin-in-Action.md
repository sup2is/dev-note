...
- 코틀린에서는 프로퍼티의 값을 바꿀 때 user.address = "new value" 처럼 필드 설정 구문을 사용한다.
...
- with 함수는 첫 번째 인자로 받은 객체를 두 번째 인자로 받은 람다의 수신 객체로 만들어준다.
...
- !!를 널에 대해 사용해서 발생하는 예외의 스택 트레이스에는 어떤 파일의 몇번째 줄인지에 대한 정보는 들어있지만 어떤 식에서 예외가 발생했는지에 대한 정보는 들어있지 않다. 어떤 값이 널이었는지 확실히 하기 위해 여러 !! 단언문을 한 줄에 함께 쓰는 일을 피하자!
...
- 코틀린 @Deprecated의 replaceWith 파라미터
  - 코틀린에서는 replaceWith 파라미터를 통해 옛 버전을 대신할 수 있는 패턴을 제시할 수 있고 API 사용자는 그 패턴을 보고 지원이 종료될 API 기능을 더 쉽게 새 버전으로 포팅할 수 있다.
  - 해당 기능은 intelliJ IDEA에서 퀵 픽스 기능으로도 제공해준다.
- 코틀린 애너테이션 적용과 자바 애너테이션 적용중 다른점
  - 클래스를 애너테이션 인자로 지정할 때는 @MyAnnotation(MyClass::class) 처럼 ::class 형태로 넣어줘야 한다.
  - 다른 애너테이션을 인자로 지정할 때는 인자로 들어가는 애너테이션의 이름 앞에 @를 넣지 말아야 한다. ReplaceWith는 애너테이션이지만 Deprecated의 인자이므로 @가 붙지 않는다.
  - 배열을 인자로 지정하려면 @RequestMapping(path = arrayOf ("foo", "bar")) 처럼 arrayOf 함수를 사용한다. 자바에서 선언한 애너테이션은 그냥 쓰면 된다.
- 애너테이션과 컴파일 타임
  - 애너테이션은 인자를 컴파일 시점에 알 수 있어야 한다. 따라서 프로퍼티는 사용 불가능하다.
  - 프로퍼티를 인자로 사용하고싶다면 const로 만들어야 한다.

### 애노테이션 대상

- 애너테이션을 붙일 때 이런 요소 중 어떤 요소에 애너테이션을 붙일지 표시할 필요가 있다.

- 사용 지점 대상

  - 사용 지점 대상 선언으로 애너테이션을 붙일 요소를 정할 수 있다.

  - 사용 지점 대상은 @기호와 애너테이션 이름 사이에 붙으며 애노테이션 이름과는 : 으로 분리된다

  - ```kotlin
    @get:Rule
    ```

- 자바에서 선언된 애너테이션을 사용해 프로퍼티에 애노테이션을 붙이는 경우 기본적으로 프로퍼티의 필드에 그 애노테이션이 붙는다. 하지만 코틀린으로 애너테이션을 선언하면 프로퍼티에 직접 적용할 수 있는 애너테이션을 만들 수 있다.

- 사용 지점 대상을 지원하는 대상 목록

  - property: 프로퍼티 전체, 자바에서 선언된 애너테이션에는 이 사용 지점 대상을 사용할 수 없다.
  - field: 프로퍼티에 의해 성성되는 필드
  - get: 프로퍼티 게터
  - set: 프로퍼티 세터
  ...
  - file: 파일 안에선언된 최상위 함수와 프로퍼티를 담아두는 클래스
...
- 직렬화
  - 객체를 저장장치에 저장하거나 네트워크를 통해 전송하기 위해 텍스트나 이진 형슥으로 변환하는 것
- 역직렬화
